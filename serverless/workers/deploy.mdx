...
The `--platform linux/amd64` flag is required to ensure compatibility with Brightnode's infrastructure.
...
Make your image available to Brightnode by pushing it to Docker Hub:
...
Once your image is in the Docker container registry, you can [create a Serverless endpoint](/serverless/endpoints/manage-endpoints#create-an-endpoint) through the Brightnode console.
...
You can deploy your worker image directly from a Docker registry through the Brightnode console:
...
1. Check the logs in the Brightnode console for error messages.
