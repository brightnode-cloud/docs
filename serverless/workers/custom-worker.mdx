...
For an even faster start, clone or download the [worker-basic](https://github.com/Brightnode-workers/worker-basic) repository for a pre-configured template for building and deploying Serverless workers. After cloning the repository, skip to [step 6 of this tutorial](#step-6%3A-build-and-push-your-docker-image) to deploy and test the endpoint.
...
* Deploy and test your worker on the Brightnode console.
...
<Steps>
  ...
  <Step title="Install the Brightnode SDK">
    ```sh
...
RUN pip install --no-cache-dir Brightnode
...
Before you can deploy your worker on Brightnode Serverless, you need to push it to Docker Hub:
<Steps>
  <Step title="Build your Docker image">
    Build your Docker image, specifying the platform for Brightnode deployment, replacing `[YOUR_USERNAME]` with your Docker username:
...
## Step 7: Deploy your worker using the Brightnode console
...
* [Manage your Serverless endpoints using the Brightnode console.](/serverless/endpoints/manage-endpoints)
