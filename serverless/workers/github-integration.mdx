...
Brightnode's GitHub integration simplifies your workflow by pulling your code and Dockerfile from GitHub, building the container image, storing it in Brightnode's secure container registry, and deploying it to your endpoint.
...
For an example repository containing the minimal files necessary for deployment, see [Brightnode-workers/worker-basic](https://github.com/Brightnode-workers/worker-basic) on GitHub.

</Tip>

## Authorizing Brightnode with GitHub

Before deploying from GitHub, you need to authorize Brightnode to access your repositories:

1. Open the [settings page](http://console.brightnode.cloud/user/settings) in the Brightnode console.
...
4. Choose which repositories Brightnode can access:
...
You can manage this connection using Brightnode settings or GitHub account settings, in the **Applications** tab.

## Deploying from GitHub

To deploy a worker from a GitHub repository:

1. Go to the [Serverless section](https://www.console.brightnode.cloud/serverless) of the Brightnode console
...
Brightnode will build your Docker image and deploy it to your endpoint automatically. You'll be redirected to the endpoint details page when complete.
...
| Status    | Description                                         |
| --------- | --------------------------------------------------- |
| Pending   | Brightnode is scheduling the build.                     |
| Building  | Brightnode is building your container.                  |
| Uploading | Brightnode is uploading your container to the registry. |
| Testing   | Brightnode is testing your Serverless worker.           |
| Completed | Brightnode completed the build and upload.              |
...
3. On the Serverless page of the Brightnode console, click the three dots to the top right of your production endpoint. Click **Clone Endpoint**.
...
1. Create a workflow file at `.github/workflows/test-and-deploy.yml`:
   ...
   test-and-deploy:
      ...
      - name: Run Tests
         uses: Brightnode/Brightnode-test-runner@v1
         with:
         image-tag: [DOCKER_USERNAME]/[WORKER_NAME]:${{ github.sha }}
         Brightnode-api-key: ${{ secrets.BRIGHTNODE_API_KEY }} # Add your API key to a GitHub secret
   ...
   To add your Brightnode API key to a GitHub secret, see [Using secrets in GitHub Actions](https://docs.github.com/en/actions/security-for-github-actions/security-guides/using-secrets-in-github-actions).
...
Brightnode's GitHub integration has a few limitations to be aware of:

## GitHub integration limitations

When using GitHub integration with Brightnode, be aware of these important limitations:
...
* **Platform exclusivity**: Images built through Brightnode's image builder service are designed exclusively for Brightnode's infrastructure and cannot be pulled or executed on other platforms.
* **Single GitHub connection**: Each Brightnode account can link to only one GitHub account. This connection cannot be shared among team members, requiring separate Brightnode accounts for collaborative projects.

## Disconnecting GitHub

To disconnect your GitHub account from Brightnode:

1. Go to [Brightnode Settings](https://www.console.brightnode.cloud/user/settings) → **Connections** → **Edit Connection**
2. Select your GitHub account.
3. Click **Configure**.
4. Scroll down to the Danger Zone.
5. Uninstall "Brightnode Inc."

## Troubleshooting deployment issues

If your worker fails to deploy or process requests:

* Check the build logs in the Brightnode console for error messages.
