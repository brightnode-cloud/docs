...
This tutorial shows how to build a vLLM application using FastAPI and deploy it as a load balancing Serverless endpoint on Brightnode.
...
You can download a preconfigured repository containing the completed code for this tutorial [on GitHub](https://github.com/Brightnode-workers/vllm-loadbalancer-ep/).
...
async def health_check():
    """Health check endpoint required by Brightnode load balancer"""
    if not engine_ready:
...
## Step 7: Deploy to Brightnode

Now, let's deploy our application to a Serverless endpoint:

1. Go to the [Serverless page](https://www.brightnode.cloud/console/serverless) in the Brightnode console.
...
<Tip>
You can find a Python script to test your vLLM load balancer locally [on GitHub](https://github.com/Brightnode-workers/vllm-loadbalancer-ep/blob/main/example.py).
</Tip>

Once your endpoint has finished deploying, you can access your vLLM APIs at:

```
https://ENDPOINT_ID.api.brightnode.ai/PATH
```

For example, the vLLM application we defined in step 4 exposes these endpoints:

- Health check: `https://ENDPOINT_ID.api.brightnode.ai/ping`
- Generate text: `https://ENDPOINT_ID.api.brightnode.ai/v1/completions`
- Chat completions: `https://ENDPOINT_ID.api.brightnode.ai/v1/chat/completions`

Use the curl commands below to make test requests to your vLLM load balancer, replacing `ENDPOINT_ID` and `BRIGHTNODE_API_KEY` with your actual values.

To run a health check:

```bash ping
curl -X GET "https://ENDPOINT_ID.api.brightnode.ai/ping" \
...
```bash
curl -X POST "https://ENDPOINT_ID.api.brightnode.ai/v1/completions" \
     -H 'Authorization: Bearer BRIGHTNODE_API_KEY' \
     -H 'Content-Type: application/json' \
...
```bash
curl -X POST "https://ENDPOINT_ID.api.brightnode.ai/v1/chat/completions" \
     -H 'Authorization: Bearer BRIGHTNODE_API_KEY' \
     -H 'Content-Type: application/json' \
     -d '{
       "messages": [
