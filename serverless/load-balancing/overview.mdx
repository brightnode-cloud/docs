...
```
https://ENDPOINT_ID.api.brightnode.ai/YOUR_CUSTOM_PATH
```
...
- Requests are processed through Brightnode's queueing system.
...
Load balancing workers do not require standardized handlers, or use the Brightnode SDK at all. Instead, you can create full REST APIs using frameworks like FastAPI:
...
```
https://ENDPOINT_ID.api.brightnode.ai/ping
https://ENDPOINT_ID.api.brightnode.ai/generate
```
...
Brightnode continuously monitors worker health through a dedicated health check mechanism. Workers must expose a `/ping` endpoint on the port specified by the `PORT_HEALTH` environment variable. The load balancer periodically sends requests to this endpoint. Workers respond with appropriate HTTP status codes:
...
When calculating endpoint metrics, Brightnode calculates the cold start time for load balancing workers by measuring the time it takes between  `/ping`  first returning `204` until it first returns `200`. 
...
def health_check_with_retry(base_url, api_key, max_retries=3, delay=2):
    """Simple health check with retry logic for Brightnode cold starts"""
...
# Usage example
base_url = "https://ENDPOINT_ID.api.brightnode.ai"
...
The load balancing system employs an HTTP load balancer that inspects application-level protocols to make routing decisions. When a request arrives at `https://ENDPOINT_ID.api.brightnode.ai/PATH`, the system:
