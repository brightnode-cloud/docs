...
This tutorial shows how to build a load balancing worker using FastAPI and deploy it as a Serverless endpoint on Brightnode.
...
You can download a preconfigured repository containing the completed code for this tutorial [on GitHub](https://github.com/Brightnode-workers/worker-load-balancing/).
...
# Health check endpoint; required for Brightnode to monitor worker health
@app.get("/ping")
...
- Generate text: `https://ENDPOINT_ID.api.brightnode.ai/generate`
- Get request count: `https://ENDPOINT_ID.api.brightnode.ai/stats`

Try running one or more of these commands, replacing `ENDPOINT_ID` and `BRIGHTNODE_API_KEY` with your actual endpoint ID and API key:

<CodeGroup>

```bash generate
curl -X POST "https://ENDPOINT_ID.api.brightnode.ai/generate" \
...
```bash stats
curl -X GET "https://ENDPOINT_ID.api.brightnode.ai/stats" \
    -H 'Authorization: Bearer BRIGHTNODE_API_KEY' \
    -H "Content-Type: application/json" \
```
