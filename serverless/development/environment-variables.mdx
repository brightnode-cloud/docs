...
This section focuses on how to use environment variables to facilitate the uploading of images to an S3 bucket using Brightnode Handler Functions.
...
* Ensure the Brightnode Python library is installed: `pip install brightnode`.
...
1. **Handler Function for S3 Upload**: Here's an example of a handler function that uploads `image.png` to an S3 bucket and returns the image URL:
   ...
   ```python
   from brightnode.serverless.utils import rp_upload
   import brightnode
...
1. **Accessing Environment Variables Setting**: In the template creation/editing interface of your bnode, navigate to the bottom section where you can set environment variables.
...
1. **Making a Request**: Make a POST request to your API endpoint with the necessary headers and input data. Remember, the input must be a JSON item:
   ...
   endpoint = "https://api.brightnode.ai/v2/xxxxxxxxx/run"
...
2. **Checking the Output**: Make a GET request to retrieve the job status and output. Here’s an example of how to do it:
   ...
   response = requests.get(
       "https://api.brightnode.ai/v2/xxxxxxxxx/status/" + response.json()["id"],
       headers=headers,
   )
   response.json()
   ```
...
By following these steps, you can effectively use environment variables to manage S3 bucket credentials and operations within your Brightnode Handler Functions. This approach ensures secure, scalable, and efficient handling of external resources in your serverless applications.
