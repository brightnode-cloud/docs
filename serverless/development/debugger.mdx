...
In the previous lesson, we covered the basics of running your Brightnode serverless functions locally. Now, let's explore some advanced options that give you more control over your local testing environment.

The Brightnode Python SDK provides several command-line arguments that allow you to customize your local testing setup.
...
The `--rp_log_level` flag enables the Brightnode debugger, which can help you troubleshoot issues in your code.
...
The following arguments allow you to configure the FastAPI server that simulates the Brightnode serverless environment:
...
```sh
python hello_world.py --rp_server_api --test_input '{"input": {"name": "Brightnode"}}'
```
...
These advanced options for local testing with the Brightnode Python SDK give you fine-grained control over your development environment. By mastering these tools, you can ensure your serverless functions are robust and ready for deployment to the Brightnode cloud.
