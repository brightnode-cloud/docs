...
description: "Create a flexible Serverless worker that supports a Bnode-first development workflow."
---
...
This tutorial shows how to build a "Bnode-first" development environment: creating a flexible, dual-mode Docker image that can be deployed as either a Bnode or a Serverless worker.

Using this method, you'll leverage a [Bnode](/bnodes/overview)—a GPU instance ideal for interactive development, with tools like Jupyter Notebooks and direct IDE integration—as your cloud-based development machine. The Bnode will be deployed with a flexible Docker base, allowing the same container image to be seamlessly deployed to a Serverless endpoint.

This workflow lets you develop and thoroughly test your application using a containerized Bnode environment, ensuring it works correctly. Then, when you're ready to deploy to production, you can deploy it instantly to Serverless.
...
To get a basic dual-mode worker up and running immediately, you can [clone this repository](https://github.com/justinwlin/Brightnode-GPU-And-Serverless-Base) and use it as a base.
...
*   Understand and utilize the "Bnode-first" development workflow.
*   Deploy and test your worker in both Bnode and Serverless environments.

## Requirements

*   You've [created a Brightnode account](/get-started/manage-accounts).
...
- `handler.py`: Your Python script with the Brightnode handler logic.
...
This Python script will contain your core logic. It will check for a user-specified environment variable `MODE_TO_RUN` to determine whether to run in Bnode or Serverless mode.
...
mode_to_run = os.getenv("MODE_TO_RUN", "bnode")
...
*   `MODE_TO_RUN = os.getenv("MODE_TO_RUN", "bnode")`: Reads the mode from an environment variable, defaulting to `bnode`.
*   `async def handler(event)`: Your core logic.
*   `if mode_to_run == "bnode" ... else`: This conditional controls what happens when the script is executed directly.
    *   In `bnode` mode, it runs a sample test call to your `handler` function, allowing for quick iteration.
    *   In `serverless`" mode, it starts the Brightnode Serverless worker.
...
export_env_vars() {
    ...
    printenv | grep -E '^BRIGHTNODE_|^PATH=|^_=' | awk -F = '{ print "export " $1 "=\"" $2 "\"" }' >> /etc/rp_environment
...
case $MODE_TO_RUN in
    serverless)
        ...
        ;;
    bnode)
        echo "Running in bnode mode"
        start_jupyter
        ;;
    *)
        echo "Invalid MODE_TO_RUN value: $MODE_TO_RUN. Expected 'serverless', 'bnode', or 'both'."
        exit 1
        ;;
esac
...
*   `serverless` mode: Executes `handler.py`, which then starts the Brightnode Serverless worker. `exec` replaces the shell process with the Python process.
*   `bnode` mode: Starts up the JupyterLab server for Bnode development, then runs `sleep infinity` to keep the container alive so you can connect to it (e.g., via SSH or `docker exec`). You would then manually run `python /app/handler.py` inside the Bnode to test your handler logic.
...
```dockerfile
# Use an official Brightnode base image
FROM Brightnode/pytorch:2.0.1-py3.10-cuda11.8.0-devel-ubuntu22.04

# Environment variables
ENV PYTHONUNBUFFERED=1 

# Supported modes: bnode, serverless
ARG MODE_TO_RUN=bnode
...
# Delete's the default start.sh file from Brightnode (so we can replace it with our own below)
RUN rm ../start.sh
...
# depot build -t justinBrightnode/bnode-server-base:1.0 . --push --platform linux/amd64
CMD $WORKSPACE_DIR/start.sh
```
Key features:
*   `FROM Brightnode/pytorch:2.0.1-py3.10-cuda11.8.0-devel-ubuntu22.04`: Starts with a Brightnode base image that comes with nginx, brightnodectl, and other helpful base packages.
...
*   `ENV MODE_TO_RUN="bnode"`: Sets the default operational mode to "bnode". This can be overridden at runtime.
...
<Steps>
  <Step title="Build your Docker image">
    ...
    ```
    The `--platform linux/amd64` flag is important for compatibility with Brightnode's infrastructure.
  </Step>
...
## Step 6: Testing in Bnode mode

Now that you've finished building our Docker image, let's explore how you would use the Bnode-first development workflow in practice.

Deploy the image to a Bnode by following these steps:

1. Go to the [Bnodes page](https://www.brightnode.cloud/console/bnodes) in the Brightnode console and click **Create Bnode**.
2. Select an appropriate GPU for your workload (see [Choose a Bnode](/bnodes/choose-a-bnode) for guidance).
3. Under **Bnode Template**, select **Edit Template**.
4. Under **Container Image**, enter `[YOUR_USERNAME]/dual-mode-worker:latest`.
5. Under **Public Environment Variables**, select **Add environment variable**. Set variable key to **`MODE_TO_RUN`** and the value to **`bnode`**.
6. Click **Set Overrides**, then deploy your Bnode.

After [connecting to the Bnode](/bnodes/connect-to-a-bnode), navigate to `/app` and run your handler directly:

```sh
python handler.py
```

This will execute the Bnode-specific test harness in your `handler.py`, giving you immediate feedback. You can edit `handler.py` within the Bnode and re-run it for rapid iteration.

## Step 7: Deploy to a Serverless endpoint

Once you're confident with your `handler.py` logic tested in Bnode mode, you're ready to deploy your dual-mode worker to a Serverless endpoint.

1. Go to the [Serverless section](https://www.brightnode.cloud/console/serverless) of the Brightnode console.
...
The *same* image is used, but `start.sh` will now direct it to run in Serverless mode, starting the `brightnode.serverless.start` worker.

## Step 8: Test your endpoint

After deploying your endpoint in to Serverless mode, you can test it with the following steps:

1.  Navigate to your endpoint's detail page in the Brightnode console.
...
## Explore the Bnode-first development workflow

Congratulations! You've successfully built, deployed, and tested a dual-mode Serverless worker. Now, let's explore the recommended iteration process for a Bnode-first development workflow:

<Steps>
  <Step title="Develop using Bnode mode">
    1. Deploy your initial Docker image to a Brightnode Bnode, ensuring `MODE_TO_RUN` is set to `bnode` (or rely on the Dockerfile default).
    2. [Connect to your Bnode](/bnodes/connect-to-a-bnode) (via SSH or web terminal).
    ...
    5. Iterate on your `handler.py` script. Test your changes frequently by running `python handler.py` directly in the Bnode's terminal. This will execute the test harness you defined in the `elif MODE_TO_RUN == "bnode":` block, giving you immediate feedback.
  </Step>
...
This iterative loop—write your handler, update the Docker image, test in Bnode mode, then deploy to Serverless—allows for rapid development and debugging of your Serverless workers.
