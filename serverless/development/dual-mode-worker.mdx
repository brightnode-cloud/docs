...
description: "Create a flexible Serverless worker that supports a Node-first development workflow."
---
...
This tutorial shows how to build a "Node-first" development environment: creating a flexible, dual-mode Docker image that can be deployed as either a Node or a Serverless worker.

Using this method, you'll leverage a [Node](/nodes/overview)—a GPU instance ideal for interactive development, with tools like Jupyter Notebooks and direct IDE integration—as your cloud-based development machine. The Node will be deployed with a flexible Docker base, allowing the same container image to be seamlessly deployed to a Serverless endpoint.

This workflow lets you develop and thoroughly test your application using a containerized Node environment, ensuring it works correctly. Then, when you're ready to deploy to production, you can deploy it instantly to Serverless.
...
*   Understand and utilize the "Node-first" development workflow.
*   Deploy and test your worker in both Node and Serverless environments.
...
This Python script will contain your core logic. It will check for a user-specified environment variable `MODE_TO_RUN` to determine whether to run in Node or Serverless mode.
...
mode_to_run = os.getenv("MODE_TO_RUN", "node")
...
*   `MODE_TO_RUN = os.getenv("MODE_TO_RUN", "node")`: Reads the mode from an environment variable, defaulting to `node`.
*   `async def handler(event)`: Your core logic.
*   `if mode_to_run == "node" ... else`: This conditional controls what happens when the script is executed directly.
    *   In `node` mode, it runs a sample test call to your `handler` function, allowing for quick iteration.
...
case $MODE_TO_RUN in
    serverless)
        ...
        ;;
    node)
        echo "Running in node mode"
        start_jupyter
        ;;
    *)
        echo "Invalid MODE_TO_RUN value: $MODE_TO_RUN. Expected 'serverless', 'node', or 'both'."
        exit 1
        ;;
esac
...
*   `node` mode: Starts up the JupyterLab server for Node development, then runs `sleep infinity` to keep the container alive so you can connect to it (e.g., via SSH or `docker exec`). You would then manually run `python /app/handler.py` inside the Node to test your handler logic.
...
# Supported modes: node, serverless
ARG MODE_TO_RUN=node
...
# depot build -t justinBrightnode/node-server-base:1.0 . --push --platform linux/amd64
...
*   `ENV MODE_TO_RUN="node"`: Sets the default operational mode to "node". This can be overridden at runtime.
...
## Step 6: Testing in Node mode

Now that you've finished building our Docker image, let's explore how you would use the Node-first development workflow in practice.

Deploy the image to a Node by following these steps:

1. Go to the [Nodes page](https://www.brightnode.cloud/console/nodes) in the Brightnode console and click **Create Node**.
2. Select an appropriate GPU for your workload (see [Choose a Node](/nodes/choose-a-node) for guidance).
3. Under **Node Template**, select **Edit Template**.
4. Under **Container Image**, enter `[YOUR_USERNAME]/dual-mode-worker:latest`.
5. Under **Public Environment Variables**, select **Add environment variable**. Set variable key to **`MODE_TO_RUN`** and the value to **`node`**.
6. Click **Set Overrides**, then deploy your Node.

After [connecting to the Node](/nodes/connect-to-a-node), navigate to `/app` and run your handler directly:

```sh
python handler.py
```

This will execute the Node-specific test harness in your `handler.py`, giving you immediate feedback. You can edit `handler.py` within the Node and re-run it for rapid iteration.

## Step 7: Deploy to a Serverless endpoint

Once you're confident with your `handler.py` logic tested in Node mode, you're ready to deploy your dual-mode worker to a Serverless endpoint.
...
## Explore the Node-first development workflow

Congratulations! You've successfully built, deployed, and tested a dual-mode Serverless worker. Now, let's explore the recommended iteration process for a Node-first development workflow:

<Steps>
  <Step title="Develop using Node mode">
    1. Deploy your initial Docker image to a Brightnode Node, ensuring `MODE_TO_RUN` is set to `node` (or rely on the Dockerfile default).
    2. [Connect to your Node](/nodes/connect-to-a-node) (via SSH or web terminal).
    ...
    5. Iterate on your `handler.py` script. Test your changes frequently by running `python handler.py` directly in the Node's terminal. This will execute the test harness you defined in the `elif MODE_TO_RUN == "node":` block, giving you immediate feedback.
  </Step>
...
This iterative loop—write your handler, update the Docker image, test in Node mode, then deploy to Serverless—allows for rapid development and debugging of your Serverless workers.
