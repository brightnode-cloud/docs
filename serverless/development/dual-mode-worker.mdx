...
# depot build -t justinBrightnode/pod-server-base:1.0 . --push --platform linux/amd64
...
1. Go to the [Pods page](https://www.brightnode.cloud/console/pods) in the Brightnode console and click **Create Pod**.
...
<Steps>
  <Step title="Develop using Pod mode">
    1. Deploy your initial Docker image to a Brightnode Pod, ensuring `MODE_TO_RUN` is set to `pod` (or rely on the Dockerfile default).
