...
When developing for Brightnode serverless, it's crucial to manage resources efficiently. The Brightnode SDK provides a `clean()` function to help you remove temporary files and folders after processing. This guide will show you how to use this cleanup utility effectively.

## The clean() Function

The `clean()` function is part of Brightnode's serverless utilities. It helps maintain a clean environment by removing specified folders and files after a job is completed.

To use it, import the function from the Brightnode serverless utilities:

```python
from brightnode.serverless.utils.rp_cleanup import clean
```
