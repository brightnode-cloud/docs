...
Brightnode Serverless is a cloud computing platform that lets you run AI models and compute-intensive workloads without managing servers. You only pay for the actual compute time you use, with no idle costs when your application isn't processing requests.

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.youtube.com/embed/5jHMLDZ4_O4"
  title="Brightnode Serverless Made Simple - Introduction To Serverless Functions and Workers"
  frameBorder="0"
...
The container instances that execute your code when requests arrive at your endpoint. Brightnode automatically manages worker lifecycle, starting them when needed and stopping them when idle to optimize resource usage.
...
brightnode.serverless.start({"handler": handler})  # Required
```
...
1. If no workers are active, Brightnode automatically starts one (cold start).
...
Brightnode Serverless offers several ways to deploy your workloads, each designed for different use cases.

### Brightnode Hub

**Best for**: Instantly deploying preconfigured AI models.

You can deploy a Serverless endpoint from a repo in the [Brightnode Hub](/hub/overview) in seconds:
...
[Deploy a repo from the Brightnode Hub →](https://www.console.brightnode.cloud/hub)
...
Brightnode maintains a collection of [worker templates](https://github.com/Brightnode-workers) on GitHub that you can use as a starting point:

* [worker-basic](https://github.com/Brightnode-workers/worker-basic): A minimal template with essential functionality.
* [worker-template](https://github.com/Brightnode-workers/worker-template): A more comprehensive template with additional features
* [Model-specific templates](https://github.com/Brightnode-workers#worker-collection): Specialized templates for common AI tasks (image generation, audio processing, etc.)
...
[Browse worker templates →](https://github.com/Brightnode-workers)
...
Ready to get started with Brightnode Serverless?
