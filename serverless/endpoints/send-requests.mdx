...
The quickest way to test your endpoint is directly in the Brightnode console. Navigate to the [Serverless section](https://www.console.brightnode.cloud/serverless), select your endpoint, and click the **Requests** tab.
...
Below you'll find detailed explanations and examples for each operation using `cURL` and the Brightnode SDK.
...
Before running these examples, you'll need to install the Brightnode SDK:
...
# Go
go get github.com/Brightnode/go-sdk && go mod tidy
```

You should also set your [API key](/get-started/api-keys) and endpoint ID (found on the Overview tab for your endpoint in the Brightnode console) as environment variables. Run the following commands in your local terminal, replacing `YOUR_API_KEY` and `YOUR_ENDPOINT_ID` with your actual API key and endpoint ID:

```bash
export BRIGHTNODE_API_KEY="YOUR_API_KEY"
...
```sh
https://api.brightnode.ai/v2/$ENDPOINT_ID/runsync?wait=120000
```
...
```sh
curl --request POST \
     --url https://api.brightnode.ai/v2/$ENDPOINT_ID/runsync \
     -H "accept: application/json" \
     -H "authorization: $BRIGHTNODE_API_KEY" \
...
endpoint = brightnode.Endpoint(os.getenv("ENDPOINT_ID"))
...
import (
	...
	"github.com/Brightnode/go-sdk/pkg/sdk"
	"github.com.Brightnode/go-sdk/pkg/sdk/config"
	rpEndpoint "github.com/Brightnode/go-sdk/pkg/sdk/endpoint"
)

func main() {
	apiKey := os.Getenv("BRIGHTNODE_API_KEY")
	baseURL := os.Getenv("BRIGHTNODE_BASE_URL")
...
```sh
curl --request POST \
     --url https://api.brightnode.ai/v2/$ENDPOINT_ID/run \
     -H "accept: application/json" \
     -H "authorization: $BRIGHTNODE_API_KEY" \
     -H "content-type: application/json" \
    -d '{"input": {"prompt": "Hello, world!"}}'
...
For example, `https://api.brightnode.ai/v2/$ENDPOINT_ID/status/YOUR_JOB_ID?ttl=6000` sets the TTL to 6 seconds.
</Tip>
...
```sh
curl --request GET \
     --url https://api.brightnode.ai/v2/$ENDPOINT_ID/status/YOUR_JOB_ID \
     -H "authorization: $BRIGHTNODE_API_KEY" \
```
...
```sh
curl --request GET \
     --url https://api.brightnode.ai/v2/$ENDPOINT_ID/stream/YOUR_JOB_ID \
     -H "accept: application/json" \
     -H "authorization: $BRIGHTNODE_API_KEY" \
```
...
```sh
curl --request POST \
  --url https://api.brightnode.ai/v2/$ENDPOINT_ID/cancel/YOUR_JOB_ID \
  -H "authorization: $BRIGHTNODE_API_KEY" \
```
</Tab>
...
```sh
curl --request POST \
     --url https://api.brightnode.ai/v2/$ENDPOINT_ID/retry/YOUR_JOB_ID \
     -H "authorization: $BRIGHTNODE_API_KEY"
```
...
```sh
curl --request POST \
     --url https://api.brightnode.ai/v2/$ENDPOINT_ID/purge-queue \
     -H "authorization: $BRIGHTNODE_API_KEY"
...
```sh
curl --request GET \
     --url https://api.brightnode.ai/v2/$ENDPOINT_ID/health \
     -H "authorization: $BRIGHTNODE_API_KEY"
```
...
[vLLM workers](/serverless/vllm/overview) are specialized containers designed to efficiently deploy and serve large language models (LLMs) on Brightnode Serverless.
...
Receive notifications when jobs complete by specifying a webhook URL. When your job completes, Brightnode will send a `POST` request to your webhook URL containing the same information as the `/status/JOB_ID` endpoint.
...
Your webhook should return a `200` status code to acknowledge receipt. If the call fails, Brightnode will retry up to 2 more times with a 10-second delay between attempts.
...
Brightnode enforces rate limits to ensure fair platform usage. These limits apply per endpoint and operation:
...
In addition to the base rate limits above, Brightnode implements a dynamic rate limiting system that scales with your endpoint's worker count. This helps ensure platform stability while allowing higher throughput as you scale.
