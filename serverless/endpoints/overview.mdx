...
Endpoints are the foundation of Brightnode Serverless, serving as the gateway for deploying and managing your Serverless workers. They provide a consistent API interface that allows your applications to interact with powerful compute resources on demand.
...
Brightnode Serverless endpoints are RESTful APIs that accept HTTP requests, execute your code, and return the result via HTTP response. Each endpoint provides a unique URL and abstracts away the complexity of managing individual GPUs/CPUs.

Behind the scenes, Brightnode handles the entire lifecycle of Serverless workers, including job queuing, execution, and result delivery, so you can focus on your code, rather than the underlying infrastructure.
...
* [Manage your endpoints using the Brightnode console.](/serverless/endpoints/manage-endpoints)
