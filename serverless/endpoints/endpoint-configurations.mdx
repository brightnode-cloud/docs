...
Choose one or more GPU categories (organized by memory) for your endpoint in order of preference. Brightnode prioritizes allocating the first category in your list and falls back to subsequent GPUs if your first choice is unavailable.
...
Enables direct communication with your worker via its public IP and port. This can be useful for real-time applications requiring minimal latency, such as [WebSocket applications](https://github.com/Brightnode-workers/worker-websocket).
...
Specify which CUDA versions can be used with your workload to ensures your code runs on compatible GPU hardware. Brightnode will match your workload to GPU instances with the selected CUDA versions.
...
Limiting your endpoint to just one or two CUDA versions can significantly reduce GPU availability. Brightnode continuously updates GPU drivers to support the latest CUDA versions, so keeping more CUDA versions selected gives you access to more resources.
...
If your worker's cold start time exceeds the default 7-minute limit (which can occur when loading large models), the system may mark it as unhealthy. To prevent this, you can extend the cold start timeout by setting the `BRIGHTNODE_INIT_TIMEOUT` environment variable. For example, setting `BRIGHTNODE_INIT_TIMEOUT=800` allows up to 800 seconds (13.3 minutes) for revival.
...
2. **Store large models on network volumes:** For flexibility during development, save large models to a [network volume](/storage/network-volumes) using a Bnode or one-time handler, then mount this volume to your Serverless workers. While network volumes offer slower model loading compared to embedding models directly, they can speed up your workflow by enabling rapid iteration and seamless switching between different models and configurations.
...
6. **Increase maximum worker limits:** Set higher maximum worker capacities to ensure your Docker images are pre-cached across multiple compute bnodes and data centers. This proactive approach eliminates image download delays during scaling events, significantly reducing startup times.
