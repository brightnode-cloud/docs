...
This tutorial shows how to deploy a large language model using Brightnode's vLLM worker. At the end, you'll have a fully functional Serverless endpoint that can handle LLM inference requests.

## What you'll learn

In this tutorial, you'll learn how to:

* Configure and deploy a vLLM worker using Brightnode Serverless.
...
* Test your endpoint using the Brightnode API.
...
## Step 2: Deploy using the Brightnode console

The easiest way to deploy a vLLM worker is through Brightnode's Ready-to-Deploy Repos:

1. Find the [vLLM repo](https://console.brightnode.cloud/hub/Brightnode-workers/worker-vllm) in the Brightnode Hub.
...
Your endpoint will now begin initializing. This may take several minutes while Brightnode provisions resources and downloads the selected model.
...
Brightnode is creating a Serverless endpoint with your specified configuration, and the vLLM worker image is being deployed using your chosen model. Once deployment is complete, make a note of your **Endpoint ID**, as you'll need this to make API requests.
...
Congratulations! You've successfully deployed a vLLM worker on Brightnode Serverless. You now have a powerful, scalable LLM inference API that's compatible with both the OpenAI client and Brightnode's native API.

Next you can try:

* [Sending requests using the Brightnode API](/serverless/vllm/vllm-requests).
