...
description: "Use Brightnode's native API to send requests to vLLM workers."
---

vLLM workers use the same request operations as any other Brightnode Serverless endpoint, with specialized input parameters for LLM inference.

## How vLLM requests work

vLLM workers are queue-based Serverless endpoints. They use the same `/run` and `/runsync` operations as other Brightnode endpoints, following the standard [Serverless request structure](/serverless/endpoints/send-requests).
...
headers = {
    "Authorization": "Bearer BRIGHTNODE_API_KEY",
...
# Stream the results
stream_url = f"https://api.brightnode.ai/v2/ENDPOINT_ID/stream/{job_id}"
