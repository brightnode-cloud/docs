...
Brightnode's vLLM workers implement OpenAI API compatibility, allowing you to use familiar [OpenAI client libraries](https://platform.openai.com/docs/libraries) with your deployed models. This guide explains how to leverage this compatibility to integrate your models with existing OpenAI-based applications.
...
```
https://api.brightnode.ai/v2/ENDPOINT_ID/openai/v1
```
...
Before you can send API requests, set up an OpenAI client with your Brightnode API key and endpoint URL:
...
# Replace ENDPOINT_ID and BRIGHTNODE_API_KEY with your actual values
client = OpenAI(
    api_key="BRIGHTNODE_API_KEY",
    base_url="https://api.brightnode.ai/v2/ENDPOINT_ID/openai/v1",
)
```

## Send requests

You can use Brightnode's OpenAI-compatible API to send requests to your Brightnode endpoint, enabling you to use the same client libraries and code that you use with OpenAI's services. You only need to change the base URL to point to your Brightnode endpoint.

<Tip>

You can also send requests using [Brightnode's native API](/serverless/vllm/vllm-requests), which provides additional flexibility and control.
...
response_stream = client.completions.create(
    model=MODEL_NAME,
    prompt="Brightnode is the best platform because",
    temperature=0,
    max_tokens=100,
    stream=True,
)
...
| `model` | `string` | Required | The model repo that you've deployed on your Brightnode Serverless endpoint. |
...
const openai = new OpenAI({
  ...
  baseURL: "https://api.brightnode.ai/v2/ENDPOINT_ID/openai/v1"
});
...
**Rate limits** follow Brightnode's endpoint policies rather than OpenAI's rate limiting structure.
...
| Authentication error      | Check that you're using your Brightnode API key, not an OpenAI key.        |
...
* [Learn how to send vLLM requests using Brightnode's native API](/serverless/vllm/vllm-requests).
