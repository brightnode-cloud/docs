...
vLLM workers make it easy to deploy and serve large language models on Brightnode Serverless, delivering fast and efficient inference with automatic scaling.

The vLLM worker image can be deployed directly from the [Brightnode Hub](https://console.brightnode.cloud/hub/Brightnode-workers/worker-vllm) or customized and built from the [GitHub repository](https://github.com/Brightnode-workers/worker-vllm).
...
The vLLM worker image includes the vLLM engine with GPU optimizations and support for both OpenAI's API and Brightnode's native API. You can deploy any supported model from Hugging Face with minimal configuration and start serving requests immediately. The workers run on Brightnode Serverless, which automatically scales based on demand.
...
1. The request arrives at Brightnode Serverless infrastructure.
...
vLLM endpoints use the same `/run` and `/runsync` operations as other Brightnode Serverless endpoints. The only difference is the input format and the specialized LLM processing inside the worker.
...
vLLM workers provide a drop-in replacement for OpenAI's API. If you're already using the OpenAI Python client or any other OpenAI-compatible library, you can switch to your Brightnode endpoint by changing just two lines of code: the API key and the base URL. Your existing prompts, parameters, and response handling code continue to work without modification.
...
There are two ways to deploy vLLM workers on Brightnode.

### Using pre-built Docker images

This is the fastest and most common approach. Brightnode provides pre-built vLLM worker images that you can deploy directly from the console. You specify your model name as an environment variable, and the worker downloads it from Hugging Face during initialization.
...
Once your endpoint is running, learn how to send requests using [Brightnode's native API](/serverless/vllm/vllm-requests) or the [OpenAI-compatible API](/serverless/vllm/openai-compatibility).
