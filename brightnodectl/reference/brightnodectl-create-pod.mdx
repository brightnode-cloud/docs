...
Create and start a new Pod on Brightnode with configuration options for GPU type, storage, networking, and cloud tier.

<RequestExample>
```sh Command
brightnodectl create pod [flags]
```
</RequestExample>
...
The Docker container image to use for the Pod (e.g., `Brightnode/pytorch:latest`).
</ResponseField>
...
- [`brightnodectl create pods`](/brightnodectl/reference/brightnodectl-create-pods)
