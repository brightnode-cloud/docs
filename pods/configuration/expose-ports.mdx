...
Brightnode provides flexible options for exposing your Pod services to the internet. This guide explains how to configure port exposure for different use cases and requirements.
...
Brightnode's HTTP proxy provides the easiest way to expose web services from your Pod. This method works well for REST APIs, web applications, and any HTTP-based service.
...
To configure HTTP ports for an existing Pod, navigate to the [Pod page](https://www.console.brightnode.cloud/pods), expand your Pod, click the hamburger menu on the bottom-left, select **Edit Pod**, then add your port(s) to the **Expose HTTP Ports (Max 10)** field.

You can also configure HTTP ports for a Pod template in the [My Templates](https://www.console.brightnode.cloud/user/templates) section of the console.
...
```
User → Cloudflare → Brightnode Load Balancer → Your Pod
```
