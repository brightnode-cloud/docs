...
Global networking creates a secure, private network that connects all your Pods within your Brightnode account. This feature enables Pod-to-Pod communication as if they were on the same local network, regardless of their physical location across different data centers.
...
1. Navigate to the [Pods](https://www.console.brightnode.cloud/pods) section and click **Deploy**. 
...
Replace `POD_ID` with the target Pod's ID. For example, if your Pod ID is `abc123xyz`, other Pods can reach it at `abc123xyz.brightnode.internal`.
