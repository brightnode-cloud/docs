...
1. Navigate to the [Pods page](https://www.console.brightnode.cloud/pods).
2. Select **Deploy**.
...
Brightnode provides an S3-compatible API that allows you to access and manage files on your network volumes directly, without needing to launch a Pod. This can be particularly useful for:
...
You can migrate files from one network volume to another using `brightnodectl send` and `receive` on two running Pods. Watch this video tutorial for a step-by-step walkthrough:
