---
title: "Manage Pods"
description: "Create, start, stop, and terminate Pods using the Brightnode console or CLI."
---

## Before you begin

If you want to manage Pods using the Brightnode CLI, you'll need to [install Brightnode CLI](/brightnodectl/overview), and set your [API key](/get-started/api-keys) in the configuration.
...
<Tip>
You can deploy preconfigured Pods from the repos listed in the [Brightnode Hub](/hub/overview). For more info, see the [Hub deployment guide](/hub/overview#deploy-as-a-pod).
</Tip>


<Tabs>
<Tab title="Web">
To create a Pod using the Brightnode console:

1. Open the [Pods page](https://www.console.brightnode.cloud/pods) in the Brightnode console and click the **Deploy** button.
...
3. (Optional) Choose a **Pod Template** such as **Brightnode Pytorch 2.1** or **Brightnode Stable Diffusion**.
...
To create a Pod using the CLI, use the `brightnodectl create pods` command:

```sh
brightnodectl create pods \
  --name hello-world \
  --gpuType "NVIDIA A40" \
...
```bash
curl --request POST \
  --url https://rest.brightnode.cloud/v1/pods \
...
Brightnode supports custom [Pod templates](/pods/templates/overview) that let you define your environment using a Dockerfile.
...
1. Open the [Pods page](https://www.console.brightnode.cloud/pods).
...
```sh
brightnodectl stop pod $BRIGHTNODE_POD_ID
```
...
```sh
sleep 2h; brightnodectl stop pod $BRIGHTNODE_POD_ID &
```

This command uses sleep to wait for 2 hours before executing the `brightnodectl stop pod` command to stop the Pod. The `&` at the end runs the command in the background, allowing you to continue using the SSH session.
...
```sh
nohup bash -c "sleep 2h; brightnodectl stop pod $BRIGHTNODE_POD_ID" &
```
...
<Tab title="Command line">
To start a single Pod, enter the command `brightnodectl start pod`. You can pass the environment variable `BRIGHTNODE_POD_ID` to identify each Pod.

```sh
brightnodectl start pod $BRIGHTNODE_POD_ID
```
...
```sh
brightnodectl remove pod $BRIGHTNODE_POD_ID
```
...
```sh
brightnodectl remove pods my-bulk-task --podCount 40
```

You can also terminate a Pod by name:

```sh
brightnodectl remove pods [POD_NAME]
```
...
You can find a list of all your Pods on the [Pods page](https://www.console.brightnode.cloud/pods) of the web interface.

If you're using the CLI, use the following command to list your Pods:

```sh
brightnodectl get pod
```

Or use this command to get the details of a single Pod:

```sh
brightnodectl get pod [POD_ID]
```
...
To view your logs, open the [Pods page](https://www.console.brightnode.cloud/pods), expand your Pod, and click the **Logs** button. This gives you real-time access to both container and system logs, making it easy to diagnose issues or monitor your Pod's activity.
...
Brightnode manages the Docker daemon for you, which means you can't run your own Docker instance inside a Pod. This prevents you from building Docker containers or using tools like Docker Compose.
