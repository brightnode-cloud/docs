...
Brightnode offers multiple flexible pricing options for Pods, designed to accommodate a variety of workloads and budgets. 


## How billing works

All Pods are billed by the second for compute and storage, with no additional fees for data ingress or egress. Every Pod has an hourly cost based on its [GPU type](/references/gpu-types) or CPU configuration, and your Brightnode credits are charged for the Pod every second it is active.

You can find the hourly cost of a specific GPU configuration on the [Brightnode console](https://www.console.brightnode.cloud/pods) during Pod deployment.

<Important>
If your account balance is projected to cover less than 10 seconds of remaining run time for your active Pods, Brightnode will pre-emptively stop all your Pods. This is to ensure your account retains a small balance, which can help preserve your data volumes. If your balance is completely drained, all Pods are subject to deletion at the discretion of the Brightnode system. We highly recommend setting up [automatic payments](https://www.console.brightnode.cloud/user/billing) to avoid service interruptions.
</Important>
...
Brightnode provides three options for Pod pricing:
...
<Warning>
You must have at least one hour's worth of time in your balance for your selected Pod configuration to rent an on-demand instance. If your balance is completely drained, all Pods are subject to deletion at the discretion of the Brightnode system.
</Warning>
...
You can select your preferred pricing model directly from the Brightnode console when configuring and deploying a new Pod.

1.  Open the [Pods page](https://www.console.brightnode.cloud/pods) in the Brightnode console and select **Deploy**.
...
5.  Click **Deploy On-Demand** (or the equivalent deployment button). If you've selected a savings plan, the upfront cost will be charged to your Brightnode credits, and your Pod will begin deploying with the discounted rate active.

## Storage billing

Brightnode offers [three types of storage](/pods/storage/types) for Pods::
...
You can monitor your active savings plans, including their associated Pods, commitment periods, and expiration dates, by visiting the dedicated [Savings plans](https://www.console.brightnode.cloud/savings-plans) section in your Brightnode console. General Pod usage and billing can be tracked through the [Billing section](https://www.console.brightnode.cloud/user/billing).
