...
Access the tool at [comfy.getbrightnode.cloud](https://comfy.getbrightnode.cloud)

ComfyUI-to-API is a web-based tool that analyzes your ComfyUI workflows and automatically generates a Dockerfile and GitHub repository for deployment on Brightnode Serverless. The tool uses a custom Comfy Agent to understand your workflow dependencies, searches for the right custom bnodes and models, and creates a complete repository that you can deploy using [Brightnode's GitHub integration](/serverless/workers/github-integration).

## What you'll learn

In this guide, you'll learn how to use ComfyUI-to-API to transform your ComfyUI workflows into Serverless workers. You'll upload a workflow file exported from ComfyUI, review the AI-powered analysis, generate a Dockerfile with all necessary dependencies, create a GitHub repository, and deploy it to Brightnode Serverless using GitHub integration.
...
- A Brightnode account for Serverless deployment.
- Basic familiarity with ComfyUI workflows and custom bnodes.
...
Visit [comfy.getbrightnode.cloud](https://comfy.getbrightnode.cloud) to access ComfyUI-to-API. The tool requires you to sign in with your GitHub account so it can create a repository on your behalf in a later step.
...
The analysis process detects custom bnodes in your workflow, locates required models and their download URLs, resolves dependencies between bnodes, and determines installation methods for each component. This typically takes 30-60 seconds depending on the complexity of your workflow.

The analysis results are displayed in expandable sections showing custom bnodes found, models identified, and any potential issues or recommendations.

## Step 4: Review the generated Dockerfile

After analysis completes, the tool generates a Dockerfile based on your workflow. Review the Dockerfile to see what will be installed. The generated Dockerfile uses the [Brightnode-workers/worker-comfyui](https://github.com/Brightnode-workers/worker-comfyui) base image, which provides a clean ComfyUI installation without any models. The Dockerfile then adds installation commands for all your custom bnodes, model download instructions with URLs, proper Python dependencies, and optimized caching layers for faster builds.

You can review and tweak the Dockerfile if needed, or skip ahead to the next step to create your GitHub repository.

<Tip>
  Review the Dockerfile carefully to ensure all your custom bnodes and models are
...
## Step 6: Deploy to Brightnode Serverless

With your GitHub repository created, you're ready to deploy to Brightnode Serverless using the [GitHub integration](/serverless/workers/github-integration). This is where Brightnode builds your Docker image from the Dockerfile and deploys it to an endpoint.

Navigate to [create a new endpoint](https://console.brightnode.cloud/serverless/new-endpoint) in the Brightnode console. Under "Import Git Repository", select your newly created repository from the dropdown menu. You'll need to connect your GitHub account to Brightnode once if you haven't already.
...
The `Dockerfile` contains all the instructions to build your worker image. It starts from the [Brightnode-workers/worker-comfyui](https://github.com/Brightnode-workers/worker-comfyui) base image, which provides a clean ComfyUI installation along with all the handler code needed to run as a Serverless endpoint. Your Dockerfile then adds your custom bnodes and downloads your required models. When you deploy via [GitHub integration](/serverless/workers/github-integration), Brightnode uses this Dockerfile to build your Docker image.
...
When you make changes to your repository, such as updating the Dockerfile to add new models or custom bnodes, Brightnode automatically detects these changes.

The [GitHub integration](/serverless/workers/github-integration) monitors your `main` branch for changes. When you push commits to `main`, Brightnode automatically rebuilds the Docker image and updates your endpoint with the new version. You can monitor the rebuild progress in the "Builds" tab of your endpoint detail page.
...
<AccordionGroup>
  ...
  <Accordion title="Dockerfile missing dependencies">
    **Problem:** The generated Dockerfile doesn't include all your custom bnodes
    or models.
    {""}
    **Solution:**
    {""}
    ...
    {""}
    2. Some very new or obscure custom bnodes may not be found automatically.
    {""}
