...
ComfyUI-to-API is a web-based tool that analyzes your ComfyUI workflows and automatically generates a Dockerfile and GitHub repository for deployment on Brightnode Serverless. The tool uses a custom Comfy Agent to understand your workflow dependencies, searches for the right custom nodes and models, and creates a complete repository that you can deploy using [Brightnode's GitHub integration](/serverless/workers/github-integration).
...
The `Dockerfile` contains all the instructions to build your worker image. It starts from the [Brightnode-workers/worker-comfyui](https://github.com/Brightnode-workers/worker-comfyui) base image, which provides a clean ComfyUI installation along with all the handler code needed to run as a Serverless endpoint. Your Dockerfile then adds your custom nodes and downloads your required models. When you deploy via [GitHub integration](/serverless/workers/github-integration), Brightnode uses this Dockerfile to build your Docker image.
...
When you make changes to your repository, such as updating the Dockerfile to add new models or custom nodes, Brightnode automatically detects these changes.
