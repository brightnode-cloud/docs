...
description: Web-based GUI for easy file browsing, uploading, downloading, and media viewing on Brightnode
icon: "party-horn"
---

# Setting up CopyParty on Brightnode

CopyParty provides a webbased GUI that makes file management simple on Brightnode instances. With its intuitive interface, you can browse directories, upload/download files, preview images and videos, and manage your Bnode's filesystem without complex command-line operations.
...
To use CopyParty on Brightnode, you need:
- **Terminal access to your Bnode** - Either through web terminal or Jupyter Labs terminal.
- An available HTTP port on your Bnode.
- A supported OS in the Docker base image.

<Note>
CopyParty has been tested on Ubuntu-based Docker images and Brightnode Official templates. When using custom templates or alternative Docker bases (like Python Slim) you may encounter file system dependency errors. Refer to the [GitHub repository](https://github.com/9001/copyparty) for details on OS support.
</Note>
...
If you see this option when clicking "Connect" on your Bnode page, you have web terminal access:
![Web Terminal Access](webterminal.png)

#### Option 2: JupyterLab terminal
If you have JupyterLab installed on your Bnode, you can access the terminal there:
![JupyterLab Terminal](labsterminal.png)

## Installation steps

### Step 1: Access your Bnode settings

Navigate to your Bnode page and locate the settings:
![Edit Bnode Settings](edit.png)

### Step 2: Add an HTTP port

<Warning>
**Adding a new port will restart your Bnode and erase any data not stored in `/workspace`**
...
Once CopyParty is running, click on the port number in your Brightnode dashboard:
...
1. **Data Persistence**: Always store important files in `/workspace` to survive bnode restarts
...
While CopyParty provides an excellent web-based solution, Brightnode also supports:
- Direct SSH/SCP transfers (if SSH is enabled)
- JupyterLab file browser
- [Brightnode CLI](/brightnodectl/overview) tool
- [Cloud storage integration](/bnodes/storage/cloud-sync) (S3, Google Drive, etc.)
