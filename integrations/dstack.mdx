---
title: "Manage Bnodes with dstack on Brightnode"
sidebarTitle: "dstack"
---

[dstack](https://dstack.ai/) is an open-source tool that simplifies the orchestration of Bnodes for AI and ML workloads. By defining your application and resource requirements in YAML configuration files, it automates the provisioning and management of cloud resources on Brightnode, allowing you to focus on your application logic rather than the infrastructure.

In this guide, we'll walk through setting up [dstack](https://dstack.ai/) with Brightnode to deploy [vLLM](https://github.com/vllm-project/vllm). We'll serve the `meta-llama/Llama-3.1-8B-Instruct` model from Hugging Face using a Python environment.

## Prerequisites

* [A Brightnode account with an API key](/get-started/api-keys)
...
1. **Prepare Your Workspace**
   ...
   ```bash
   mkdir Brightnode-dstack-tutorial
   cd Brightnode-dstack-tutorial
   ```
...
### Configuring dstack for Brightnode
...
1. **Create the Global Configuration File**
   ...
   * **Create the `config.yml` File**
     ...
     Replace `YOUR_BRIGHTNODE_API_KEY` with the API key you obtained from Brightnode.
...
* The `ports` configuration provides port forwarding from the deployed bnode to `localhost`, allowing you to access the deployed vLLM via `localhost:8000`.

3. **Monitor the Deployment**

* After executing `dstack apply`, you'll see all the steps that dstack performs:

  * Provisioning the bnode on Brightnode.
...
Since the `ports` configuration forwards port `8000` from the deployed bnode to `localhost`, you can access the vLLM server via `http://localhost:8000`.
...
curl -X POST http://localhost:8000/v1/chat/completions \
     ...
     -d '{
          ...
          "messages": [
             {"role": "system", "content": "You are Bnodedy, a helpful assistant."},
...
```bash
curl -X POST http://localhost:8000/v1/chat/completions -H "Content-Type: application/json" -d "{ \"model\": \"meta-llama/Llama-3.1-8B-Instruct\", \"messages\": [ {\"role\": \"system\", \"content\": \"You are Bnodedy, a helpful assistant.\"}, {\"role\": \"user\", \"content\": \"What is your name?\"} ], \"temperature\": 0, \"max_tokens\": 150 }"
```
...
curl.exe -Method Post http://localhost:8000/v1/chat/completions `
  ...
  -Body '{ "model": "meta-llama/Llama-3.1-8B-Instruct", "messages": [ {"role": "system", "content": "You are Bnodedy, a helpful assistant."}, {"role": "user", "content": "What is your name?"} ], "temperature": 0, "max_tokens": 150 }'
```
  ...
  "choices": [
    {
      ...
      "message": {
        "role": "assistant",
        "content": "My name is Bnodedy, and I'm here to assist you with any questions or information you may need.",
        "tool_calls": []
      },
      "logprobs": null,
      "finish_reason": "stop",
...
Check your Brightnode dashboard or the [dstack](https://dstack.ai/) web UI to ensure that the instance has been terminated.
...
The `region` ties your volume to a specific region, which then also ties your Bnode to that same region.
...
For more information on using volumes with Brightnode, refer to the [dstack blog on volumes](https://dstack.ai/blog/volumes-on-Brightnode/).

***

## Conclusion

By leveraging [dstack](https://dstack.ai/) on Brightnode, you can efficiently deploy and manage Bnodes, accelerating your development workflow and reducing operational overhead.
