---
title: "Manage Nodes with dstack on Brightnode"
sidebarTitle: "dstack"
---

[dstack](https://dstack.ai/) is an open-source tool that simplifies the orchestration of Nodes for AI and ML workloads. By defining your application and resource requirements in YAML configuration files, it automates the provisioning and management of cloud resources on Brightnode, allowing you to focus on your application logic rather than the infrastructure.
...
* The `ports` configuration provides port forwarding from the deployed node to `localhost`, allowing you to access the deployed vLLM via `localhost:8000`.

3. **Monitor the Deployment**

* After executing `dstack apply`, you'll see all the steps that dstack performs:

  * Provisioning the node on Brightnode.
...
Since the `ports` configuration forwards port `8000` from the deployed node to `localhost`, you can access the vLLM server via `http://localhost:8000`.
...
curl -X POST http://localhost:8000/v1/chat/completions \
     ...
     -d '{
          ...
          "messages": [
             {"role": "system", "content": "You are Nodedy, a helpful assistant."},
...
```bash
curl -X POST http://localhost:8000/v1/chat/completions -H "Content-Type: application/json" -d "{ \"model\": \"meta-llama/Llama-3.1-8B-Instruct\", \"messages\": [ {\"role\": \"system\", \"content\": \"You are Nodedy, a helpful assistant.\"}, {\"role\": \"user\", \"content\": \"What is your name?\"} ], \"temperature\": 0, \"max_tokens\": 150 }"
```
...
curl.exe -Method Post http://localhost:8000/v1/chat/completions `
  ...
  -Body '{ "model": "meta-llama/Llama-3.1-8B-Instruct", "messages": [ {"role": "system", "content": "You are Nodedy, a helpful assistant."}, {"role": "user", "content": "What is your name?"} ], "temperature": 0, "max_tokens": 150 }'
```
  ...
  "choices": [
    {
      ...
      "message": {
        "role": "assistant",
        "content": "My name is Nodedy, and I'm here to assist you with any questions or information you may need.",
        "tool_calls": []
      },
      "logprobs": null,
      "finish_reason": "stop",
...
The `region` ties your volume to a specific region, which then also ties your Node to that same region.
...
By leveraging [dstack](https://dstack.ai/) on Brightnode, you can efficiently deploy and manage Nodes, accelerating your development workflow and reducing operational overhead.
