---
title: "Running Brightnode on Mods"
sidebarTitle: "Mods"
---
...
1. **Obtain Your API Key**:
   ...
   * Visit the [Brightnode Settings](https://www.console.brightnode.cloud/user/settings) page to retrieve your API key.
...
3. **Configure Brightnode**:

   * Update the `config_template.yml` file to use your Brightnode configuration. Here's an example:

     ```yml
     Brightnode:
       # https://docs.brightnode.cloud/serverless/vllm/openai-compatibility
       base-url: https://api.brightnode.ai/v2/${YOUR_ENDPOINT}/openai/v1
       api-key:
       api-key-env: BRIGHTNODE_API_KEY
       models:
         # Add your model name
   ...
   * `api-key-env`: Add your Brightnode API key.
...
4. **Verify Your Setup**:
   ...
   * Specify the Brightnode API and model you want to use.

     ```sh
     ls ~/Downloads | mods --api Brightnode --model openchat -f "tell my fortune based on these files" | glow
     ```

   * This command will list the files in your `~/Downloads` directory, pass them to Mods using the Brightnode API and the specified model, and format the response as a fortune based on the files. The output will then be piped to `glow` for a visually appealing display.
