---
title: "Brightnode Secure Cloud partner requirements (2025)"
sidebarTitle: "Secure Cloud partner requirements"
---

## Introduction

This document outlines the specifications required to be a Brightnode secure cloud partner. These requirements establish the baseline, however for new partners, Brightnode will perform a due diligence process prior to selection encompassing business health, prior performance, and corporate alignment.
...
Each datacenter must have a storage cluster which provides shared storage between all GPU servers. The hardware is provided by the partner, storage cluster licensing is provided by Brightnode. All storage servers must be accessible by all GPU compute machines.
...
| Component                          | Requirement                                                                      |
| ---------------------------------- | -------------------------------------------------------------------------------- |
| Redundancy                         | None (JBOD) - Brightnode will assemble into array. 7 to 14TB disk sizes recommended. |
| Disk Perf - Sequential read        | 6,000 MB/s                                                                       |
| Disk Perf - Sequential write       | 5,000 MB/s                                                                       |
| Disk Perf - Random Read (4K QD32)  | 400,000 IOPS                                                                     |
| Disk Perf - Random Write (4K QD32) | 40,000 IOPS                                                                      |
...
Once a storage cluster exceeds 90% single core CPU on the leader bnode during peak hours, a dedicated metadata server is required. Metadata tracking is a single process operation, and single threaded performance is the most important metric.
...
Each datacenter should have a CPU server that to accommodate CPU-only Bnodes and Serverless workers. Brightnode will also use this server to host additional features for which a GPU is not required (e.g., the [S3-compatible API](/serverless/storage/s3-api)).
...
| Testing and Maintenance | - Monthly generator tests under load for a minimum of 30 minutes<br/>- Quarterly full-load tests of the entire backup power system, including UPS and generators<br/>- Annual full-facility power outage test (coordinated with Brightnode)<br/>- Regular thermographic scanning of electrical systems<br/>- Detailed maintenance logs for all power equipment<br/>- 24/7 on-site facilities team for immediate response to power issues |
...
| Access Layer            | - Redundant top-of-rack switches in each cabinet<br/>- Minimum 100 Gbps server connections for high-performance compute bnodes                                                                                                                                                                                                                                  |
...
To qualify as a Brightnode secure cloud partner, the parent organization must adhere to at least one of the following compliance standards:
...
| Physical security | Brightnode servers must be held in an isolated secure rack or cage in an area that is not accessible to any non-partner or approved DC personnel. Physical access to this area must be tracked and logged. |
| Maintenance       | All maintenance resulting in disruption or downtime must be scheduled at least 1 week in advance. Large disruptions must be coordinated with Brightnode at least 1 month in advance.                       |

Brightnode will review evidence of:
