...
<Steps>
  <Step title="Open your Pod in the console">
    Go to the [Pods section](https://www.brightnode.cloud/console/pods) in the Brightnode console, then find your deployed ComfyUI Pod and expand it.
...
- **HTTP service not ready**: Wait at least 2 to 3 minutes after Pod deployment for the HTTP service to fully start. You can also check the Pod logs in the Brightnode console to look for deployment errors.
...
Don't forget to install any missing models using the model manager. If you need a model that isn't available in the model manager, you can download it from the web to your local machine, then use the [Brightnode CLI](/brightnodectl/overview) to transfer the model files directly into your Pod's `/workspace/madapps/ComfyUI/models` directory.
...
While working with ComfyUI, you can monitor your usage by checking GPU/disk utilization in the [Pods page](https://console.brightnode.cloud/pods) of the Brightnode console.
