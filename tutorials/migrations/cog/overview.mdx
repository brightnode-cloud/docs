...
title: "Cog"
sidebar_label: Cog
description: Migrate your Cog model to Brightnode
---
...
In this tutorial, you'll go through the process of migrating a model deployed via replicate.com or utilizing the Cog framework to a Brightnode serverless worker.

This guide assumes you are operating within a Linux terminal environment and have Docker installed on your system.

<Info>

This method might occur a delay when working with Brightnode Serverless Endpoints. This delay is due to the FastAPI server that is used to run the Cog model.

To eliminate this delay, consider using [Brightnode Handler](/serverless/workers/handler-functions) functions in a future iteration.
...
```bash
git clone https://github.com/Brightnode-workers/cog-worker.git
cd cog-worker/
```
...
Now that you have identified the necessary information about your model, you can proceed to build and push your Docker image. This is a crucial step, as it prepares your model for deployment on the Brightnode platform.
...
With your Docker image built and pushed, you're one step closer to deploying your Cog model on Brightnode.

## Create and Deploy a Serverless Endpoint

Now that your Docker image is ready, it's time to create and deploy a serverless endpoint on Brightnode. This step will enable you to send requests to your new endpoint and use your Cog model in a serverless environment.

To create and deploy a serverless endpoint on Brightnode:
...
Congratulations, you have successfully migrated your Cog model from Replicate to Brightnode and set up a serverless endpoint. As you continue to develop your models and applications, consider exploring additional features and capabilities offered by Brightnode to further enhance your projects.

Here are some resources to help you continue your journey:

* [Learn more about Brightnode serverless workers](/serverless/overview)
* [Explore additional Brightnode tutorials and examples](/tutorials/introduction/overview)
