...
These imports bring in the `Brightnode` SDK for serverless functions and the `pipeline` method from `transformers`, which allows us to use pre-trained models.
...
To run our sentiment analysis function as a serverless worker, we need to start the worker using Brightnode's SDK. Add the following line at the end of your `sentiment_analysis.py` file:

```python sentiment_analysis.py
brightnode.serverless.start({"handler": sentiment_analysis_handler})
```
...
{
  "input": {
    "text": "I love using Brightnode for serverless machine learning!"
  }
}
```
...
DEBUG  | Retrieved local job: {'input': {'text': 'I love using Brightnode for serverless machine learning!'}, 'id': 'local_test'}
...
In this tutorial, you learned how to integrate a pre-trained Hugging Face model into a Brightnode serverless function to perform sentiment analysis on text input.
