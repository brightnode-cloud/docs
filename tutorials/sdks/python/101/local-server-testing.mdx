---
title: "Creating and testing a Brightnode serverless function with local server"
sidebarTitle: "Local server testing"
---

This tutorial will guide you through creating a basic serverless function using Brightnode's Python SDK. We'll build a function that reverses a given string, demonstrating the simplicity and flexibility of Brightnode's serverless architecture.

## Setting up your Serverless Function

Let's break down the process of creating our string reversal function into steps.

### Import Brightnode Library

First, import the Brightnode library:
...
Finally, start the Brightnode serverless worker:

```python
brightnode.serverless.start({"handler": handler})
```

This line registers our handler function with Brightnode's serverless infrastructure.
...
Brightnode provides multiple ways to test your serverless function locally before deployment. We'll explore two methods: using command-line arguments and running a local test server.
...
```bash
python your_script.py --test_input '{"input": {"text": "Hello, Brightnode!"}}'
```
...
DEBUG  | Retrieved local job: {'input': {'text': 'Hello, Brightnode!'}, 'id': 'local_test'}
INFO   | local_test | Started.
string-reverser | Starting job local_test
DEBUG  | local_test | Handler output: {'original_text': 'Hello, Brightnode!', 'reversed_text': '!doPnuR ,olleH'}
DEBUG  | local_test | run_job return: {'output': {'original_text': 'Hello, Brightnode!', 'reversed_text': '!doPnuR ,olleH'}}
INFO   | Job local_test completed successfully.
INFO   | Job result: {'output': {'original_text': 'Hello, Brightnode!', 'reversed_text': '!doPnuR ,olleH'}}
...
* Exploring Brightnode's documentation for advanced features like concurrent processing or GPU acceleration

Brightnode's serverless library provides a powerful foundation for building scalable, efficient text processing applications without the need to manage infrastructure.
