...
Brightnode's serverless library enables you to create and deploy scalable functions without managing infrastructure. This tutorial will walk you through creating a simple serverless function that determines whether a number is even.

## Creating a Basic Serverless Function

Let's start by building a function that checks if a given number is even.

### Import the Brightnode library
...
Wrap your function with `brightnode.serverless.start()`:

```python is_even.py
brightnode.serverless.start({"handler": is_even})
```
...
You've now created a basic serverless function using Brightnode's Python SDK. This approach allows for efficient, scalable deployment of functions without the need to manage infrastructure.
...
* Exploring Brightnode's documentation for advanced features and best practices

Brightnode's serverless library provides a powerful tool for a wide range of applications, from simple utilities to complex data processing tasks.
