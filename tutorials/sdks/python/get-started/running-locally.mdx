...
Before deploying your serverless functions to the cloud, it's crucial to test them locally. In the previous lesson, [Hello World with Brightnode](/tutorials/sdks/python/get-started/hello-world), you created a Python file called `hello_world.py`.

In this guide, you'll learn how to run your Brightnode serverless applications on your local machine using the Brightnode Python SDK.

## Understanding Brightnode's Local Testing Environment

When you run your code locally using the Brightnode Python SDK, here's what happens behind the scenes:

* FastAPI Server: The SDK spins up a FastAPI server on your local machine. This server simulates the Brightnode serverless environment.
...
* Environment Simulation: The local setup mimics key aspects of the Brightnode serverless environment, helping ensure your code will behave similarly when deployed.

## Running Your Code Locally

Let's walk through how to run your serverless functions locally using the Brightnode Python SDK.

**Options for Passing Information to Your API**

The Brightnode Python SDK offers two main methods for sending data to your local FastAPI server:
...
* Local testing with the Brightnode Python SDK allows you to simulate the cloud environment on your machine.
...
Next, we'll explore the structure of Brightnode handlers in more depth, enabling you to create more sophisticated serverless functions.
