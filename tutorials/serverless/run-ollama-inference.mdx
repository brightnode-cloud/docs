---
title: "Run an Ollama Server on a Brightnode CPU"
---

In this guide, you will learn how to run an Ollama server on your Brightnode CPU for inference. Although this tutorial focuses on CPU compute, you can also select a GPU type and follow the same steps. By the end of this tutorial, you will have a fully functioning Ollama server ready to handle requests.
...
To begin, you need to set up a new endpoint on Brightnode.

1. Log in to your [Brightnode account](https://www.console.brightnode.cloud/console/home).
...
5. In the **Container Image** field, enter the `pooyaharatian/Brightnode-ollama:0.0.8` container image.
...
8. (optional) In **Environment Variables**, set a new key to `OLLAMA_MODELS` and its value to `/Brightnode-volume`. This will allow the model to be stored to your attached volume.
...
1. Go to the **Requests** section in the Brightnode web UI.
...
In this tutorial, you have successfully set up and run an Ollama server on a Brightnode CPU. Now you can handle inference requests using your deployed model.

For further exploration, check out the following resources:

* [Brightnode Ollama repository](https://github.com/pooyahrtn/)
* [Brightnode Ollama container image](https://hub.docker.com/r/pooyaharatian/Brightnode-ollama)
