...
description: "Learn how to deploy a Serverless endpoint running ComfyUI from the Brightnode Hub and use it to generate images with FLUX Dev."
tag: "NEW"
---

In this tutorial, you will learn how to deploy a Serverless endpoint running [ComfyUI](https://github.com/comfyanonymous/ComfyUI) on Brightnode, submit image generation jobs using workflow JSON, monitor their progress, and decode the resulting images.

[Brightnode's Serverless platform](/serverless/overview) allows you to run AI/ML models in the cloud without managing infrastructure, automatically scaling resources as needed. ComfyUI is a powerful bnode-based interface for Stable Diffusion that provides fine-grained control over the image generation process through customizable workflows.

## What you'll learn

In this tutorial you'll learn:

- How to deploy a ComfyUI Serverless endpoint using the [Brightnode Hub](/hub/overview).
...
- A Brightnode account with available credits.
- A Brightnode API key (available in your user settings).
...
## Step 1: Deploy a ComfyUI Serverless endpoint using the Brightnode Hub
...
If you want to use a different model, you can also [deploy the endpoint](https://github.com/Brightnode-workers/worker-comfyui/blob/main/docs/deployment.md) using one of these pre-defined Docker images:

- `Brightnode/worker-comfyui:<version>-base` - Clean ComfyUI install with no models.
- `Brightnode/worker-comfyui:<version>-flux1-schnell` - FLUX.1 schnell model.
- `Brightnode/worker-comfyui:<version>-flux1-dev` - FLUX.1 dev model.
- `Brightnode/worker-comfyui:<version>-sdxl` - Stable Diffusion XL model.
- `Brightnode/worker-comfyui:<version>-sd3` - Stable Diffusion 3 medium model.

Replace `<version>` with the latest release version from GitHub Releases.

If you need a model that's not listed here, or have your own LoRA, or need custom bnodes, you can use this [customization guide](https://github.com/Brightnode-workers/worker-comfyui/blob/main/docs/customization.md) to create your own custom worker.
</Accordion>

1. Navigate to the [ComfyUI Hub listing](https://console.brightnode.cloud/hub/Brightnode-workers/worker-comfyui) in the Brightnode web interface.
...
Once deployed, your endpoint will be assigned a unique ID (e.g. `32vgrms732dkwi`). Your endpoint URL will follow this pattern: `https://api.brightnode.ai/v2/ENDPOINT_ID/run` for asynchronous requests.

## Step 2: Prepare your ComfyUI workflow

ComfyUI uses workflow JSON to define the image generation process. The workflow contains bnodes that represent different steps in the generation pipeline, such as loading models, encoding prompts, and saving images.
...
- **Bnode 6**: Encodes the positive text prompt using CLIP.
- **Bnode 30**: Loads the FLUX.1-dev-fp8 checkpoint.
- **Bnode 31**: Performs the sampling process with specified parameters.
- **Bnode 8**: Decodes the latent image to a viewable format.
- **Bnode 9/40**: Saves the generated image.

You can customize the prompt by modifying the `text` field in bnode 6, or adjust generation parameters like `steps`, `cfg`, `width`, and `height` in their respective bnodes.
...
Replace `ENDPOINT_ID` with your actual endpoint ID and `YOUR_API_KEY` with your Brightnode API key in the following command:

```bash
curl -X POST https://api.brightnode.ai/v2/ENDPOINT_ID/run \
...
```bash
curl https://api.brightnode.ai/v2/ENDPOINT_ID/status/JOB_ID \
    -H 'Content-Type: application/json' \
    -H 'Authorization: Bearer YOUR_API_KEY'
```
...
Congratulations! You've successfully used Brightnode's Serverless platform to generate an AI image using ComfyUI with the FLUX.1-dev-fp8 model. You now understand the complete workflow for submitting ComfyUI jobs, monitoring their progress, and retrieving results.

## Understanding ComfyUI workflows

ComfyUI workflows are JSON structures that define the image generation pipeline through interconnected bnodes. Each bnode has:

- **Inputs**: Parameters and connections to other bnodes.
- **Class type**: The operation this bnode performs.
- **Meta information**: Human-readable titles and descriptions.

You can create custom workflows by modifying bnode parameters or [opening the ComfyUI interface in a Bnode](/tutorials/bnodes/comfyui) and exporting the workflow to JSON.
...
Now that you've learned how to generate images with ComfyUI on Serverless, you can explore advanced configuration options for the ComfyUI Serverless worker on the [Brightnode-workers/worker-comfyui](https://github.com/Brightnode-workers/worker-comfyui) GitHub repository.
