...
This tutorial walks you through running Google's Gemma model using Brightnode's vLLM Worker. Throughout this tutorial, you'll learn to set up a Serverless Endpoint with a gated large language model (LLM).
...
The next section will guide you through Setting up your Serverless Endpoint with Brightnode.

## Get started

To begin, we'll deploy a vLLM Worker as a Serverless Endpoint. Brightnode simplifies the process of running large language models, offering an alternative to the more complex Docker and Kubernetes deployment methods.

Follow these steps in the Brightnode Serverless console to create your Endpoint.
...
3. Provide the following:
   ...
   v. Enter the vLLM Worker image: `Brightnode/worker-vllm:stable-cuda11.8.0` or `Brightnode/worker-vllm:stable-cuda12.1.0`.
...
* `api_key`: Your Brightnode API key.
...
client = OpenAI(
    base_url=os.environ.get("BRIGHTNODE_BASE_URL"),
...
Set your environment variables `BRIGHTNODE_BASE_URL` and `BRIGHTNODE_API_KEY` to your Brightnode API key and base URL. Your `BRIGHTNODE_BASE_URL` will be in the form of:

```
https://api.brightnode.ai/v2/${BRIGHTNODE_ENDPOINT_ID}/openai/v1
```

Where `${BRIGHTNODE_ENDPOINT_ID}` is the ID of your Serverless Endpoint.
...
Congratulations! You've successfully set up a Serverless Endpoint and interacted with Google's Gemma model. This tutorial has shown you the essentials of deploying a model on Brightnode and creating a simple application to communicate with it. You've taken important steps towards integrating large language models into your projects.
