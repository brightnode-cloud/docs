...
When it comes to working with an AI image generator, the speed in which images are generated is often a compromise. Brightnode's Serverless Workers allows you to host [SDXL Turbo](https://huggingface.co/stabilityai/sdxl-turbo) from Stability AI, which is a fast text-to-image model.

In this tutorial, you'll build a web application, where you'll leverage Brightnode's Serverless Worker and Endpoint to return an image from a text-based input.

By the end of this tutorial, you'll have an understanding of running a Serverless Worker on Brightnode and sending requests to an Endpoint to receive a response.
...
To continue with this quick start, you'll need access to the following from Brightnode:
...
You can also use the prebuilt image from [Brightnode/sdxl-turbo](https://hub.docker.com/r/Brightnode/sdxl-turbo).
...
This step will walk you through building and pushing your Docker image to your container registry. This is useful to building custom images for your use case. If you prefer, you can use the prebuilt image from [Brightnode/sdxl-turbo](https://hub.docker.com/r/Brightnode/sdxl-turbo) instead of building your own.

Building a Docker image allows you to specify the container when creating a Worker. The Docker image includes the [Brightnode Handler](https://github.com/Brightnode-workers/worker-sdxl-turbo/blob/main/src/handler.py) which is how you provide instructions to Worker to perform some task. In this example, the Handler is responsible for taking a Job and returning a base 64 instance of the image.

1. Clone the [Brightnode Worker SDXL Turbo](https://github.com/Brightnode-workers/worker-sdxl-turbo) repository:

```bash
gh repo clone Brightnode-workers/worker-sdxl-turbo
```
...
Now that you've pushed your container registry, you're ready to deploy your Serverless Endpoint to Brightnode.
...
This step will walk you through deploying a Serverless Endpoint to Brightnode.

1. Log in to the [Brightnode Serverless console](https://www.console.brightnode.cloud/serverless).
...
3. Provide the following:
   ...
   6. Enter the name of your Docker image.
      * For example, `Brightnode/sdxl-turbo:dev`.
...
<Tab title="cURL">
```bash
curl -X POST "https://api.brightnode.ai/v2/${YOUR_ENDPOINT}/runsync" \
...
<head>
    ...
    <title>Brightnode AI Image Generator</title>
    <style>
        body {
...
<body>
    <h1>Brightnode AI Image Generator</h1>
...
The JavaScript file (`script.js`) contains the `generateImage` function. This function reads the user's input, makes a POST request to the Brightnode serverless endpoint, and handles the response. The server's response is expected to contain the base64-encoded image, which is then displayed on the webpage.
...
async function generateImage() {
  ...
  try {
    const response = await fetch(
      ...
      "https://api.brightnode.ai/v2/${process.env.REACT_APP_ENDPOINT_ID}/runsync",
      options,
    );
...
This web application serves as a basic example of how to interact with your Brightnode serverless endpoint from a client-side application. It can be expanded or modified to fit more complex use cases.
