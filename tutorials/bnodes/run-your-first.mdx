...
This tutorial shows how to deploy a Bnode and use JupyterLab to generate text with the SmolLM3 model using the Python `transformers` library.
...
- Deploy a Bnode with the PyTorch template.
...
- At least $5 in Brightnode credits.
- Basic familiarity with Python and Jupyter notebooks.

## Step 1: Deploy a Bnode with PyTorch template

First, you'll deploy a Bnode using the official Brightnode PyTorch template:

1. Navigate to the [Bnodes page](https://console.brightnode.cloud/bnodes) in the Brightnode console.
2. Click **Deploy** to create a new Bnode.
3. In the template selection, choose latest the **Brightnode PyTorch** template (this should be the default setting).
...
6. Click **Deploy On-Demand** to create your Bnode.

Wait for your Bnode to initialize. This typically takes 2-5 minutes.

## Step 2: Install required packages

Once your Bnode is running, you'll need to install the `transformers` and `accelerate` Python libraries:

1. In the Brightnode console, find and expand your deployed Bnode and click **Connect**.
...
1. Go back to the Brightnode console and click **Connect** on your Bnode again.
...
<Tip>
JupyterLab is one of several [built-in utility services](/bnodes/utility-services) included with every Bnode. You'll also find VS Code Server, system monitoring, file browser, and other tools pre-configured and ready to use.
</Tip>
...
The first time you run this code, it will download the SmolLM3 model (approximately 6 GB), which may take a minute or two depending on your Bnode's internet connection. Subsequent runs will be much faster, as the model will be cached locally.
...
- **JupyterLab not accessible**: Wait a few minutes after Bnode deployment for services to fully start. If the JupyterLab tab is blank when you open it, try stopping and then restarting the Bnode.
...
- **Persistent storage**: If you plan to work with SmolLM3 regularly, consider using a [network volume](/storage/network-volumes) to persist your models and notebooks across Bnode sessions.