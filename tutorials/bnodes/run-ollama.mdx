---
title: "Set up Ollama on your GPU Bnode"
---

This tutorial will guide you through setting up [Ollama](https://ollama.com), a powerful platform serving large language model, on a GPU Bnode using Brightnode. Ollama makes it easy to run, create, and customize models.
...
In the following tutorial, you'll set up a Bnode on a GPU, install and serve the Ollama model, and interact with it on the CLI.
...
1. Log in to your [Brightnode account](https://www.console.brightnode.cloud/bnodes) and choose **+ GPU Bnode**.
...
Once the Bnode is up and running, you'll have access to a terminal within the Brightnode interface.
...
This command fetches the Ollama installation script and executes it, setting up Ollama on your Bnode. The `ollama serve` part starts the Ollama server, making it ready to serve AI models.

Now that your Ollama server is running on your Bnode, add a model.
...
With Ollama set up and running, you can now interact with it using HTTP API requests. In step 1.4, you configured Ollama to listen on all network interfaces. This means you can use your Bnode as a server to receive requests.
...
<Tab title="cURl">
```bash
curl https://{POD_ID}-11434.brightnode.cloud/api/tags
# curl https://cmko4ns22b84xo-11434.brightnode.cloud/api/tags
```
...
Getting a list of available models is great, but how do you send an HTTP request to your Bnode?

**Make requests**

To make an HTTP request against your Bnode, you can use the Ollama interface with your Bnode Id.

```javascript
curl -X POST https://{POD_ID}-11434.brightnode.cloud/api/generate -d '{
  "model": "mistral",
...
Because port `11434` is exposed, you can make requests to your Bnode using the `curl` command.
...
This tutorial provides a foundational understanding of setting up and using Ollama on a GPU Bnode with Brightnode.

* **Port Configuration and documentation**: For further details on exposing ports and the link structure, refer to the [Brightnode documentation](/bnodes/configuration/expose-ports).
