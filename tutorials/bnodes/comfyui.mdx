...
This tutorial walks you through how to configure ComfyUI on a [GPU Bnode](/bnodes/overview) and use it to generate images with text-to-image models.

[ComfyUI](https://www.comfy.org/) is a bnode-based graphical interface for creating AI image generation workflows. Instead of writing code, you connect different components visually to build custom image generation pipelines. This approach provides flexibility to experiment with various models and techniques while maintaining an intuitive interface.
...
<Tip>
When you're just getting started with ComfyUI, it's important to use a workflow that was created for the specific model you intend to use. You usually can't just switch the "Load Checkpoint" bnode from one model to another and expect optimal performance or results.
...
## Step 1: Deploy a ComfyUI Bnode

First, you'll deploy a Bnode using a template that pre-installs ComfyUI and the ComfyUI Manager plugin:

<Steps>
  <Step title="Deploy a Bnode from the ComfyUI template">
    ...
    <Note>
    There are several ComfyUI templates available in the Brightnode console, many with pre-installed models. We're using this one because it's extremely lightweight, but it comes with the ComfyUI Manager plugin pre-installed so you can easily install the specific models/bnodes you need after deployment.
    </Note>
  </Step>

  <Step title="Configure your Bnode">
    Configure your Bnode with these settings:
    ...
    - **Storage:** The default container and volume disk sizes set by the template should be sufficient for SDXL-Turbo. You can also add a [network volume](/storage/network-volumes) to your Bnode if you want persistent storage.
    - **Deployment type:** Select **On-Demand** for flexibility.
  </Step>

  <Step title="Deploy and wait">
    Click **Deploy On-Demand** to create your Bnode.
    
    The Bnode can take up to 30 minutes to initialize the container and start the ComfyUI HTTP service.
  </Step>
</Steps>

## Step 2: Open the ComfyUI interface

Once your Bnode has finished initializing, you can open the ComfyUI interface:

<Steps>
  <Step title="Open your Bnode in the console">
    Go to the [Bnodes section](https://www.brightnode.cloud/console/bnodes) in the Brightnode console, then find your deployed ComfyUI Bnode and expand it.

    <Warning>
    The Bnode may take up to 30 minutes to initialize when first deployed. Future starts will generally take much less time.
    </Warning>
  </Step>

  <Step title="Start the ComfyUI service">
    Click **Connect** on your Bnode, then select the last HTTP service button in the list, labeled **Connect to HTTP Service [Port 8188]**.

    This will open the ComfyUI interface in a new browser tab. The URL follows the format: `https://[POD_ID]-8188.brightnode.cloud`.
...
ComfyUI workflows consist of a series of bnodes that are connected to each other to create a AI generation pipeline. Rather than creating our own workflow from scratch, we'll load a pre-configured workflow that was created for the specific model we intend to use:
...
As soon as you load the workflow, you'll see a popup labeled **Missing Models**. This happens because the Bnode template we deployed doesn't come pre-installed with any models, so we'll need to install them now.

Rather than clicking the download button (which downloads the missing model to your local machine), use the ComfyUI Manager plugin to install the missing model directly onto the Bnode:
...
<Steps>
  ...
  <Step title="Configure the checkpoint bnode">
    Find the bnode labeled **Load Checkpoint** in the workflow. It should be the first bnode on the left side of the canvas.
...
<Steps>
  <Step title="Customize your prompt">
    Locate the text input bnode labeled **CLIP Text Encode (Prompt)** in the workflow. 
  ...
  <Step title="Start generation">
    ...
    Watch as the workflow progresses through each bnode:
    - Text encoding.
    - Model loading.
  ...
  <Step title="View your result">
    The generated image appears in the output bnode when complete. 
...
- **Connection errors**: Wait for the Bnode to fully initialize (up to 30 minutes for initial deployment).
- **HTTP service not ready**: Wait at least 2 to 3 minutes after Bnode deployment for the HTTP service to fully start. You can also check the Bnode logs in the Brightnode console to look for deployment errors.
- **Out of memory errors**: Reduce image resolution or batch size in your workflow.
- **Slow generation**: Make sure you're using an appropriate GPU for your selected model. See [Choose a Bnode](/bnodes/choose-a-bnode) for guidance.
...
Don't forget to install any missing models using the model manager. If you need a model that isn't available in the model manager, you can download it from the web to your local machine, then use the [Brightnode CLI](/brightnodectl/overview) to transfer the model files directly into your Bnode's `/workspace/madapps/ComfyUI/models` directory.

### Create custom workflows

Build your own workflows by:

1. Right-clicking the canvas to add new bnodes.
2. Connecting bnode outputs to inputs by dragging between connection points.
3. Saving your custom workflow with <kbd>Ctrl</kbd>+<kbd>S</kbd> or by clicking **Workflow** and selecting **Save**.

### Manage your Bnode

While working with ComfyUI, you can monitor your usage by checking GPU/disk utilization in the [Bnodes page](https://console.brightnode.cloud/bnodes) of the Brightnode console.

Stop your Bnode when you're finished to avoid unnecessary charges.

It's also a good practice to download any custom workflows to your local machine before stopping the Bnode. For persistent storage of models and outputs across sessions, consider using a [network volume](/storage/network-volumes).