---
title: "Fine tune an LLM with Axolotl on Brightnode"
---

<Info>

Brightnode provides an easier method to fine tune an LLM. For more information, see [Fine tune a model](/fine-tune/).

</Info>

[axolotl](https://github.com/OpenAccess-AI-Collective/axolotl) is a tool that simplifies the process of training large language models (LLMs). It provides a streamlined workflow that makes it easier to fine-tune AI models on various configurations and architectures. When combined with Brightnode's GPU resources, Axolotl enables you to harness the power needed to efficiently train LLMs.
...
In this tutorial, we'll walk through the steps of training an LLM using Axolotl on Brightnode and uploading your model to Hugging Face.

### Setting up the environment

Fine-tuning a large language model (LLM) can take up a lot of compute power. Because of this, we recommend fine-tuning using Brightnode's GPUs.

To do this, you'll need to create a Bnode, specify a container, then you can begin training. A Bnode is an instance on a GPU or multiple GPUs that you can use to run your training job. You also specify a Docker image like `axolotlai/axolotl-cloud:main-latest` that you want installed on your Bnode.

1. Login to [Brightnode](https://www.console.brightnode.cloud/console/home) and deploy your Bnode.
...
For optimal compatibility, we recommend using A100, H100, V100, or RTX 3090 Bnodes for Axolotl fine-tuning.

</Note>

Now that you have your Bnode set up and running, connect to it over secure SSH.

2. Wait for the Bnode to startup, then connect to it using secure SSH.

   1. On your Bnode page, select **Connect**.
   2. Copy the secure SSH string and paste it into your terminal on your machine.

   ```bash
   ssh <username>@<bnode-ip-address> -p <ssh-port> -i <path-to-ssh-key>  string
   ```

   Follow the on-screen prompts to SSH into your Bnode.

<Info>

You should use the SSH connection to your Bnode as it is a persistent connection. The Web UI terminal shouldn't be relied on for long-running processes, as it will be disconnected after a period of inactivity.

</Info>

With the Bnode deployed and connected via SSH, we're ready to move on to preparing our dataset.
...
To use a local dataset, you'll need to transfer it to your Brightnode instance. You can do this using Brightnode CLI to securely transfer files from your local machine to the one hosted by Brightnode. All Bnodes automatically come with `brightnodectl` installed with a Bnode-scoped API key. **To send a file**

<Tabs>
<Tab title="brightnodectl">
Run the following on the computer that has the file you want to send, enter the following command:

```bash
brightnodectl send data.jsonl
```
...
<Tabs>
<Tab title="brightnodectl">
The following is an example of a command you'd run on your Brightnode machine.
...
Once the local dataset is transferred to your Brightnode machine, we can proceed to updating requirements and preprocessing the data.
...
Now update your Brightnode machine's requirement and preprocess your data.
...
In some cases, your Bnode will not contain the Axolotl repository. To add the required repository, run the following commands and then continue with the tutorial:
...
By following these steps and leveraging the power of Axolotl and Brightnode, you can efficiently fine-tune LLMs to suit your specific use cases. The combination of Axolotl's user-friendly interface and Brightnode's GPU resources makes the process more accessible and streamlined. Remember to explore the provided YAML examples to gain a deeper understanding of the various parameters and make appropriate adjustments for your own projects. With practice and experimentation, you can unlock the full potential of fine-tuned LLMs and create powerful, customized AI models.
