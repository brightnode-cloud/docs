...
To do this, you'll need to create a Node, specify a container, then you can begin training. A Node is an instance on a GPU or multiple GPUs that you can use to run your training job. You also specify a Docker image like `axolotlai/axolotl-cloud:main-latest` that you want installed on your Node.

1. Login to [Brightnode](https://www.console.brightnode.cloud/console/home) and deploy your Node.
...
For optimal compatibility, we recommend using A100, H100, V100, or RTX 3090 Nodes for Axolotl fine-tuning.

</Note>

Now that you have your Node set up and running, connect to it over secure SSH.

2. Wait for the Node to startup, then connect to it using secure SSH.

   1. On your Node page, select **Connect**.
   2. Copy the secure SSH string and paste it into your terminal on your machine.

   ```bash
   ssh <username>@<node-ip-address> -p <ssh-port> -i <path-to-ssh-key>  string
   ```

   Follow the on-screen prompts to SSH into your Node.

<Info>

You should use the SSH connection to your Node as it is a persistent connection. The Web UI terminal shouldn't be relied on for long-running processes, as it will be disconnected after a period of inactivity.

</Info>

With the Node deployed and connected via SSH, we're ready to move on to preparing our dataset.
...
To use a local dataset, you'll need to transfer it to your Brightnode instance. You can do this using Brightnode CLI to securely transfer files from your local machine to the one hosted by Brightnode. All Nodes automatically come with `brightnodectl` installed with a Node-scoped API key. **To send a file**
...
In some cases, your Node will not contain the Axolotl repository. To add the required repository, run the following commands and then continue with the tutorial:
