...
This tutorial shows how to deploy a Node and use JupyterLab to generate text with the SmolLM3 model using the Python `transformers` library.
...
- Deploy a Node with the PyTorch template.
...
## Step 1: Deploy a Node with PyTorch template

First, you'll deploy a Node using the official Brightnode PyTorch template:

1. Navigate to the [Nodes page](https://console.brightnode.cloud/nodes) in the Brightnode console.
2. Click **Deploy** to create a new Node.
...
6. Click **Deploy On-Demand** to create your Node.

Wait for your Node to initialize. This typically takes 2-5 minutes.

## Step 2: Install required packages

Once your Node is running, you'll need to install the `transformers` and `accelerate` Python libraries:

1. In the Brightnode console, find and expand your deployed Node and click **Connect**.
...
1. Go back to the Brightnode console and click **Connect** on your Node again.
...
The first time you run this code, it will download the SmolLM3 model (approximately 6 GB), which may take a minute or two depending on your Node's internet connection. Subsequent runs will be much faster, as the model will be cached locally.
...
- **JupyterLab not accessible**: Wait a few minutes after Node deployment for services to fully start. If the JupyterLab tab is blank when you open it, try stopping and then restarting the Node.
...
- **Persistent storage**: If you plan to work with SmolLM3 regularly, consider using a [network volume](/storage/network-volumes) to persist your models and notebooks across Node sessions.