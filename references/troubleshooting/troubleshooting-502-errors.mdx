...
502 errors can occur when users attempt to access a program running on a specific port of a deployed bnode and the program isn't running or has encountered an error. This document provides guidance to help you troubleshoot this error.

### Check your Bnode's GPU

The first step to troubleshooting a 502 error is to check whether your bnode has a GPU attached.

1. **Access your bnode's settings**: Click on your bnode's settings in the user interface to access detailed information about your bnode.

2. **Verify GPU attachment**: Here, you should be able to see if your bnode has a GPU attached. If it does not, you will need to attach a GPU.

If a GPU is attached, you will see it under the Bnodes screen (e.g. 1 x A6000). If a GPU is not attached, this number will be 0. Brightnode does allow you to spin up a bnode with 0 GPUs so that you can connect to it via a Terminal or CloudSync to access data. However, the options to connect to Brightnode via the web interface will be nonfunctional, even if they are lit up.

<Frame caption="">
  <img src="/images/1950025a-fb4c0dd-image-a00848a71ff50d8ce10e5e7f35cc33ff.png" />
</Frame>

### Check your Bnode's logs

After confirming that your bnode has a GPU attached, the next step is to check your bnode's logs for any errors.

1. **Access your bnode's logs**: You can view the logs from the bnode's settings in the user interface.
