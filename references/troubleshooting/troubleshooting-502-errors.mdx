...
502 errors can occur when users attempt to access a program running on a specific port of a deployed node and the program isn't running or has encountered an error. This document provides guidance to help you troubleshoot this error.

### Check your Node's GPU

The first step to troubleshooting a 502 error is to check whether your node has a GPU attached.

1. **Access your node's settings**: Click on your node's settings in the user interface to access detailed information about your node.

2. **Verify GPU attachment**: Here, you should be able to see if your node has a GPU attached. If it does not, you will need to attach a GPU.

If a GPU is attached, you will see it under the Nodes screen (e.g. 1 x A6000). If a GPU is not attached, this number will be 0. Brightnode does allow you to spin up a node with 0 GPUs so that you can connect to it via a Terminal or CloudSync to access data. However, the options to connect to Brightnode via the web interface will be nonfunctional, even if they are lit up.
...
After confirming that your node has a GPU attached, the next step is to check your node's logs for any errors.

1. **Access your node's logs**: You can view the logs from the node's settings in the user interface.
