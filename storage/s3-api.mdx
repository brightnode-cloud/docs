...
description: "Use Brightnode's S3-compatible API to access and manage your network volumes."
---

Brightnode provides an S3-protocol compatible API for direct access to your [network volumes](/storage/network-volumes). This allows you to manage files on your network volumes without launching a Bnode, reducing cost and operational friction.
...
|------------|--------------|
| `EUR-IS-1` | `https://s3api-eur-is-1.brightnode.cloud/` |
| `EU-RO-1`  | `https://s3api-eu-ro-1.brightnode.cloud/`  |
| `EU-CZ-1`  | `https://s3api-eu-cz-1.brightnode.cloud/`  |
| `US-KS-2`  | `https://s3api-us-ks-2.brightnode.cloud/`  |
| `US-CA-2` | `https://s3api-us-ca-2.brightnode.cloud/`  |
...
<Step title="Create an S3 API key">
    Next, you'll need to generate a new key called an "S3 API key" (this is separate from your Brightnode API key).

    1. Go to the [Settings page](https://www.console.brightnode.cloud/user/settings) in the Brightnode console.
    ...
    For security, Brightnode will show your API key secret only once, so you may wish to save it elsewhere (e.g., in your password manager, or in a GitHub secret). Treat your API key secret like a password and don't share it with anyone.
  ...
  <Step title="Configure AWS CLI">
    ...
    To use the S3-compatible API with your Brightnode network volumes, you must configure your AWS CLI with the Brightnode S3 API key you created.
    ...
    3.  Provide the following when prompted:
        *   **AWS Access Key ID**: Enter your Brightnode user ID. You can find this in the [Secrets section](https://www.console.brightnode.cloud/user/secrets) of the Brightnode console, in the description of your S3 API key. By default, the description will look similar to: `Shared Secret for user_2f21CfO73Mm2Uq2lEGFiEF24IPw 1749176107073`. `user_2f21CfO73Mm2Uq2lEGFiEF24IPw` is the user ID (yours will be different).
        *   **AWS Secret Access Key**: Enter your Brightnode S3 API key's secret access key.
        *   **Default Region name**: You can leave this blank.
        *   **Default output format**: You can leave this blank or set it to `json`.

    This will configure the AWS CLI to use your Brightnode S3 API key by storing these details in your AWS credentials file (typically at `~/.aws/credentials`).
...
You can use the S3-compatible API to interact with your Brightnode network volumes using standard S3 tools:
...
Network volumes are mounted to Serverless workers at `/Brightnode-volume` and to Bnodes at `/workspace` by default. The S3-compatible API maps file paths as follows:

- **Bnode filesystem path**: `/workspace/my-folder/file.txt`
- **Serverless worker path**: `/Brightnode-volume/my-folder/file.txt`
...
Unlike traditional S3 key-value stores, object names in the Brightnode S3-compatible API correspond to actual file paths on your network volume. Object names containing special characters (e.g., `#`) may need to be URL-encoded to ensure proper processing.
...
aws s3 ls --region DATACENTER \
    --endpoint-url https://s3api-DATACENTER.brightnode.cloud/ \ 
...
You can also use the Boto3 library to interact with the S3-compatible API, using it to transfer files to and from a Brightnode network volume.

The script below demonstrates how to upload a file to a Brightnode network volume using the Boto3 library. It takes command-line arguments for the network volume ID (as an S3 bucket), the datacenter-specific S3 endpoint URL, the local file path, the desired object (file path on the network volume), and the AWS Region (which corresponds to the Brightnode datacenter ID).

<AccordionGroup>

<Accordion title="Boto3 script">

To run this script, your Brightnode S3 API key credentials must be set as environment variables using the values from the [Setup and authentication](#setup-and-authentication) step:

*   `AWS_ACCESS_KEY_ID`: Should be set to your Brightnode S3 API key **access key** (e.g., `user_***...`).
*   `AWS_SECRET_ACCESS_KEY`: Should be set to your Brightnode S3 API key's **secret** (e.g., `rps_***...`).
...
import os
import argparse
import boto3 # AWS SDK for Python, used to interact with Brightnode S3-compatible APIs

def create_s3_client(region: str, endpoint_url: str):

    # Creates and returns an S3 client configured for Brightnode network volume S3-compatible API.
    #
    # Args:
    #   region (str): The Brightnode datacenter ID, used as the AWS region
    #                 (e.g., 'ca-qc-1').
    #   endpoint_url (str): The S3 endpoint URL for the specific Brightnode datacenter
    #                       (e.g., 'https://ca-qc-1-s3api.brightnode.cloud/').

    # Returns:
    #   boto3.client: An S3 client object, configured for the Brightnode S3 API.

    # Retrieve Brightnode S3 API key credentials from environment variables.
    ...
    if not aws_access_key_id or not aws_secret_access_key:
        raise EnvironmentError(
            ...
            "These are obtained from 'S3 API Keys' in the Brightnode console settings."
        )

    # Initialize and return the S3 client for Brightnode's S3-compatible API
    return boto3.client(
        "s3",
        aws_access_key_id=aws_access_key_id,
        aws_secret_access_key=aws_secret_access_key,
        region_name=region, # Corresponds to the Brightnode datacenter ID
...
def put_object(s3_client, bucket_name: str, object_name: str, file_path: str):
    ...
    # Uploads a local file to the specified Brightnode network volume.
    #
    # Args:
    #   s3_client: The S3 client object (e.g., returned by create_s3_client).
    #   bucket_name (str): The ID of the target Brightnode network volume.
    ...
    try:
        # Attempt to upload the file to the Brightnode network volume.
...
def main():
    ...
    # to a Brightnode network volume.

    # Set up command-line argument parsing
    parser = argparse.ArgumentParser(
        description="Upload a file to a Brightnode Network Volume using its S3-compatible API. "
                    "Requires AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY env vars to be set "
                    "with your Brightnode S3 API key credentials."
    )
    parser.add_argument(
        "-b", "--bucket",
        required=True,
        help="The ID of your Brightnode Network Volume (acts as the S3 bucket name)."
    )
    parser.add_argument(
        "-e", "--endpoint",
        required=True,
        help="The S3 endpoint URL for your Brightnode datacenter (e.g., 'https://s3api-DATACENTER.brightnode.cloud/')."
    )
    parser.add_argument(
        "-f", "--file",
        required=True,
    ...
    parser.add_argument(
        "-r", "--region",
        required=True,
        help="The Brightnode datacenter ID, used as the AWS region (e.g., 'ca-qc-1'). Find this in the Brightnode console's Storage section or endpoint URL."
    )

    args = parser.parse_args()

    # Create the S3 client using the parsed arguments, configured for Brightnode.
...
```bash
./s3_example_put.py --endpoint https://s3api-eur-is-1.brightnode.cloud/ \
    --region 'EUR-IS-1' \
...
You can upload large files to network volumes using S3 multipart upload operations (see the [compatibility reference](#s3-api-compatibility-reference) below). You can also download [this helper script](https://github.com/Brightnode/Brightnode-s3-examples/blob/main/upload_large_file.py), which dramatically improves reliability when uploading very large files (10GB+) by handling timeouts and retries automatically.

[Click here to download the script on GitHub.](https://github.com/Brightnode/Brightnode-s3-examples/blob/main/upload_large_file.py)
...
./upload_large_file.py --file /path/to/large/file.mp4 \
     ...
     --endpoint https://s3api-eur-is-1.brightnode.cloud/ \
     --region EUR-IS-1
```
...
| Operation | Supported | CLI Command | Notes |
|------------------|--------|-----------------|-------|
| `CreateBucket` | ❌ | `aws s3api create-bucket` | Use the Brightnode console to create network volumes |
| `DeleteBucket` | ❌ | `aws s3api delete-bucket` | Use the Brightnode console to delete network volumes |
| `GetBucketLocation` | ❌ | `aws s3api get-bucket-location` | Datacenter info available in the Brightnode console |
...
This occurs because Brightnode must compute and cache the MD5 checksum (i.e., ETag) for files created without the S3-compatible API. This computation can take several minutes for large directories or files, as the `ListObjects` request must wait until the checksum is ready.
...
-   **Object names**: Unlike traditional S3 key-value stores, object names in the Brightnode S3-compatible API correspond to actual file paths on your network volume. Object names containing special characters (e.g., `#`) may need to be URL encoded to ensure proper processing.
...
The `aws s3 sync` command has limited support in Brightnode's S3-compatible API. While it works for basic use cases (syncing small numbers of files and simple directory structures), you may encounter errors when syncing directories with very large numbers of files (over 10,000) or complex nested structures.
...
aws s3 cp large-file.zip s3://your-volume-name/ --cli-read-timeout 7200 --endpoint-url https://storage.datacenter.brightnode.cloud
```
