...
- **Persistent data that outlives compute resources**: Your data remains accessible even after Nodes are terminated or Serverless workers stop.
- **Shareable storage**: Share data across multiple Nodes or Serverless endpoints by attaching the same network volume.
- **Portable storage**: Move your working environment and data between different compute resources.
- **Efficient data management**: Store frequently used models or large datasets to avoid re-downloading them for each new Node or worker, saving time, bandwidth, and reducing cold start times.
...
1. Navigate to the [Serverless section](https://www.console.brightnode.cloud/serverless/user/endpoints) of the Brightnode console.
...
Data from the network volume will be accessible to all workers for that endpoint from the `/Brightnode-volume` directory. Use this path to read and write shared data in your [handler function](/serverless/workers/handler-functions).
...
Network volumes must be attached during Node deployment. They cannot be attached to a previously-deployed Node, nor can they be detached later without deleting the Node.

To deploy a Node with a network volume attached:

1. Navigate to the [Nodes section](https://www.console.brightnode.cloud/nodes) of the Brightnode console.
2. Select **Deploy**.
3. Select **Network Volume** and choose the network volume you want to attach from the dropdown list.
4. Select a GPU type. The system will automatically show which Nodes are available to use with the selected network volume.
...
Data from the network volume will be accessible to the Node from the volume mount path (default: `/workspace`). Use this directory to upload, download, and manipulate data that you want to share with other Nodes.
...
Network volumes for Instant Clusters work the same way as they do for Nodes. They must be attached during cluster creation, and by default are mounted at `/workspace` within each node in the cluster.

### Attach to an Instant Cluster

To enable workers on an Instant Cluster to use a network volume:

1. Navigate to the [Instant Clusters section](https://www.console.brightnode.cloud/cluster) of the Brightnode console.
...
Brightnode provides an [S3-compatible API](/storage/s3-api) that allows you to access and manage files on your network volumes directly, without needing to launch a Node or run a Serverless worker for file management. This is particularly useful for:
...
The simplest way to migrate files between network volumes is to use `brightnodectl send` and `receive` on two running Nodes.
...
<Steps>
<Step title="Deploy Nodes with network volumes attached">
Deploy two Nodes using the default Brightnode PyTorch template. Each Node should have one [network volume attached](#attach-to-a-node).

1. Deploy the first Node in the source data center and attach the source network volume.
2. Deploy the second Node in the target data center and attach the target network volume.
3. Start the [web terminal](/nodes/connect-to-a-node#web-terminal) in both Nodes.
</Step>

<Step title="Open the source volume">
Using your source Node's web terminal, navigate to the network volume directory (usually `/workspace`):
...
Using your destination Node's web terminal, navigate to the network volume directory (usually `/workspace`):
...
For faster migration speed and more reliability for large transfers, you can use `rsync` over SSH on two running Nodes.
...
<Step title="Set up SSH keys on the source Node">
On the source Node, install required packages and generate an SSH key pair:
...
On the destination Node, install required packages and add the source Node's public key to `authorized_keys`:
...
In the editor that opens, paste the public key you copied from the source Node, then save and exit (press `Esc`, type `:wq`, and press `Enter`).

The command above also displays the `rsync` command you'll need to run on the source Node. Copy this command for the next step.
</Step>

<Step title="Run the rsync command">
On the source Node, run the `rsync` command from the previous step. If you didn't copy it, you can construct it manually using the destination Node's IP address and port number.

```bash
# Replace DESTINATION_PORT and DESTINATION_IP with values from the destination Node
...
After the `rsync` command completes, verify the data transfer by checking disk usage on both Nodes:

```bash
du -sh /workspace
```

The destination Node should show similar disk usage to the source Node if all files transferred successfully.
