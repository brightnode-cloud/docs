...
TMUX is a terminal multiplexer that creates persistent terminal sessions on your Bnode.
...
To use TMUX, you need terminal access to your Brightnode Bnode (SSH, web terminal, or JupyterLab terminal).
...
TMUX is not installed by default on Brightnode Bnodes. To install it, run:
...
<Note>
Most Brightnode templates are based on Ubuntu/Debian images that support `apt-get`. If you're using Alpine Linux, use `apk add tmux` instead. Some minimal images may lack required dependencies.
</Note>
...
# You can now safely disconnect from the Bnode

# Later, reconnect to the Bnode and reattach
...
Your processes will continue running even if you disconnect from the Bnode.
...
When using TMUX on Brightnode Bnodes, keep these tips in mind:
...
- Consider creating a TMUX session immediately when connecting to a Bnode for important work.

## Troubleshooting

If you can't reattach to a session after reconnecting to a Bnode, the Bnode may have been restarted. TMUX sessions don't persist across Bnode restarts, so ensure you save your work regularly and use network volumes for persistent storage.