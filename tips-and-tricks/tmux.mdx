...
TMUX is a terminal multiplexer that creates persistent terminal sessions on your Node.
...
To use TMUX, you need terminal access to your Brightnode Node (SSH, web terminal, or JupyterLab terminal).
...
TMUX is not installed by default on Brightnode Nodes. To install it, run:
...
# You can now safely disconnect from the Node

# Later, reconnect to the Node and reattach
...
Your processes will continue running even if you disconnect from the Node.
...
When using TMUX on Brightnode Nodes, keep these tips in mind:
...
- Consider creating a TMUX session immediately when connecting to a Node for important work.

## Troubleshooting

If you can't reattach to a session after reconnecting to a Node, the Node may have been restarted. TMUX sessions don't persist across Node restarts, so ensure you save your work regularly and use network volumes for persistent storage.