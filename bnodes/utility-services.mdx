...
description: "Access development tools, monitoring, and ML services included with every Pod."
---

Every Runpod Pod comes with a suite of pre-configured utility services that provide web-based development environments, system monitoring, and machine learning tools. These services run automatically when your Pod starts and are accessible via secure HTTPS URLs.

All utility services are accessed through your browser using unique hostnames in the format `https://[service]-[pod-id].proxy.runpod.net`. No additional configuration or installation is required—everything works out of the box.

## Available services

The specific services available depend on which template you're using. All Pods include base services, while specialized templates include additional tools.

### Base services

These services are available in all Pod templates.
...
**Access:** Find the **Jupyter Lab** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.

**Authentication:** Password-protected. Set your password using the `JUPYTER_PASSWORD` environment variable when deploying your Pod, or use the default password shown in your Pod's logs.
...
**Access:** Find the **VS Code** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.

**Authentication:** Password-protected. The system auto-generates a password (visible in your Pod logs), or you can set your own using the `CODE_SERVER_PASSWORD` environment variable.
...
A real-time system monitoring dashboard powered by Glances that displays CPU, GPU, memory, disk, and network statistics. Monitor your Pod's resource utilization at a glance to optimize performance and troubleshoot issues.

**Access:** Find the **Monitor** or **Glances** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
A web-based file manager that lets you upload, download, and manage files on your Pod without using command-line tools. File Browser provides a simple interface for transferring data and organizing your workspace.

**Access:** Find the **Files** or **File Browser** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
<Note>
File Browser operates on your entire Pod filesystem. Your persistent data is typically stored in `/workspace`.
</Note>
...
**Access:** Find the **TensorBoard** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
**Access:** Find the **Gradio** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
After editing, restart the Gradio service or restart your Pod to see your changes.
...
**Access:** Find the **Streamlit** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
After editing, restart the Streamlit service or restart your Pod to see your changes.
...
**Access:** Find the **API** or **vLLM** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
client = OpenAI(
    base_url="https://api-[your-pod-id].proxy.runpod.net/v1",
...
**Access:** Find the **Chat** or **Open WebUI** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
**Access:** Find the **ComfyUI** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
**Access:** Find the **Gallery** link under **HTTP Services** in the **Connect** tab of your Pod detail pane.
...
All HTTP services follow a consistent URL pattern based on your Pod ID. When you open the **Connect** tab in your Pod detail pane, you'll see all available services with clickable links.

**URL format:** `https://[service]-[pod-id].proxy.runpod.net`

For example, if your Pod ID is `2s56cp0pof1rmt`:
- JupyterLab: `https://jupyter-2s56cp0pof1rmt.proxy.runpod.net`
- VS Code: `https://vscode-2s56cp0pof1rmt.proxy.runpod.net`
- Monitor: `https://monitor-2s56cp0pof1rmt.proxy.runpod.net`
- Files: `https://files-2s56cp0pof1rmt.proxy.runpod.net`
...
Some services require authentication. You can set credentials when creating your Pod using environment variables.

### During Pod creation

When deploying a Pod through the web console:

1. Navigate to the [Pod creation page](https://console.runpod.io/pod/create).
...
When creating a Pod via the API, include environment variables in your request:
...
<Warning>
If you don't set a password for password-protected services, the system will auto-generate one. You can find auto-generated passwords in your Pod's logs (accessible via the **Logs** tab in the Pod detail pane).
</Warning>
...
```bash
ssh root@[pod-id].proxy.runpod.net
scp local-file.py root@[pod-id].proxy.runpod.net:/workspace/
```
...
All HTTP services use HTTPS with automatic TLS certificates. Your data is encrypted in transit between your browser and the Pod.
...
Some services like Glances (system monitor), File Browser, TensorBoard, and demo apps don't require authentication. These services are isolated per user—only you can access your Pod's services using your unique Pod ID in the URL.

<Warning>
Never share your Pod's service URLs publicly if they contain sensitive data. Each URL includes your unique Pod ID and provides direct access to your services.
</Warning>
...
3. Add the public key to your Pod's `PUBLIC_KEY` environment variable when deploying.
...
1. Verify your Pod is in the **Running** state (not starting, stopping, or error).
2. Check the **Connect** tab for the correct service URL.
3. Wait 30-60 seconds after Pod startup for all services to initialize.
...
4. Verify the service is running by SSHing into your Pod and checking processes:
...
1. Check the **Logs** tab in your Pod detail pane—auto-generated passwords appear in the logs.
2. Alternatively, SSH into your Pod and restart the service with a new password:
...
Now that you understand the utility services available with your Pods, you might want to:

- Learn how to [connect to your Pod](/pods/connect-to-a-pod) using different methods.
- Explore [Pod templates](/pods/templates/overview) to find the right pre-configured environment.
- Set up [persistent storage](/pods/storage/types) for your projects.
- Review [environment variables](/pods/templates/environment-variables) for customizing your Pod.
