...
Every Brightnode Pod comes with a suite of pre-configured utility services that provide web-based development environments, system monitoring, and machine learning tools. These services run automatically when your Pod starts and are accessible via secure HTTPS URLs.

All utility services are accessed through your browser using unique hostnames in the format `https://[service]-[pod-id].brightnode.cloud`. No additional configuration or installation is required—everything works out of the box.
...
client = OpenAI(
    base_url="https://api-[your-pod-id].brightnode.cloud/v1",
...
**URL format:** `https://[service]-[pod-id].brightnode.cloud`
...
1. Navigate to the [Pod creation page](https://console.brightnode.cloud/pod/create).
...
```bash
curl -X POST https://api.brightnode.io/v2/pods \
...
```bash
ssh root@[pod-id].brightnode.cloud
scp local-file.py root@[pod-id].brightnode.cloud:/workspace/
```
