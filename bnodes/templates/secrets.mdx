...
description: "Securely store and manage sensitive information like API keys, passwords, and tokens with Brightnode secrets."
---

This guide shows how to create, view, edit, delete, and use secrets in your [Bnode templates](/bnodes/templates/overview) to protect sensitive data and improve security.

## What are Brightnode secrets

Secrets are encrypted strings that store sensitive information separately from your template configuration, providing a secure way to store and manage sensitive information such as API keys, passwords, and authentication tokens in your Bnode templates. 
...
- **Reusability**: The same secret can be referenced across multiple templates and Bnodes without duplication.
...
You can create secrets through the Brightnode web interface to securely store sensitive information:

1. Go to the [Secrets](https://www.console.brightnode.cloud/user/secrets) section in the Brightnode console.
...
1. Go to the [Secrets](https://www.console.brightnode.cloud/user/secrets) section.
...
<Warning>
Deleting a secret is permanent and cannot be undone. Make sure no active templates or Bnodes are using the secret before deletion, as this will cause those deployments to fail.
</Warning>

## Using secrets in Bnode templates

Once you've created secrets, you can reference them in your Bnode templates to provide secure access to sensitive data.

### Direct reference method

Reference your secrets directly in the [environment variables section](/bnodes/templates/environment-variables) of your Bnode template using the `BRIGHTNODE_SECRET_` prefix followed by your secret name:
...
```
{{ BRIGHTNODE_SECRET_huggingface_token }}
```

This syntax tells Brightnode to substitute the encrypted secret value when the Bnode starts, making it available as an environment variable inside your container.

### Web interface selection

When creating or editing a Bnode template through the web interface, you can also:
...
- **Environment variable mapping**: Map secrets to appropriately named environment variables in your templates:
   ```
   API_KEY={{ BRIGHTNODE_SECRET_openai_key }}
   DATABASE_URL={{ BRIGHTNODE_SECRET_db_connection }}
   ```
