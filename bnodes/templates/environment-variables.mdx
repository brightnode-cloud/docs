...
description: "Learn how to use environment variables in Brightnode Bnodes for configuration, security, and automation"
---
...
- Reference [Brightnode secrets](/bnodes/templates/secrets) in your containers.
...
**Bnode metadata access**: Brightnode provides [predefined environment variables](#Brightnode-provided-environment-variables) that give your application information about the Bnode's environment, resources, and network configuration. This metadata helps your application adapt to its runtime environment automatically.
...
1. When creating a new Bnode, click **Edit Template** and expand the **Environment Variables** section.
...
4. Save the template for reuse across multiple Bnodes.

### Using secrets

For sensitive data, you can reference [Brightnode secrets](/bnodes/templates/secrets) in environment variables using the `BRIGHTNODE_SECRET_` prefix. For example:
...
1. Navigate to the [Bnodes](https://www.console.brightnode.cloud/user/bnodes) section of the console.
2. Click the three dots to the right of the Bnode you want to update and select **Edit Bnode**.
...
For more details, see [Expose ports](/bnodes/configuration/expose-ports#symmetrical-port-mapping).
...
- **Never hardcode secrets**: Use [Brightnode secrets](/bnodes/templates/secrets) for sensitive data.
