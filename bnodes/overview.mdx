...
- [From a template](https://docs.brightnode.io/pods/templates/overview): Pre-configured environments for quick setup of common workflows.
...
- **From Serverless repos**: Deploy any Serverless-compatible repository from the [Brightnode Hub](/hub/overview#deploy-as-a-pod) directly as a Pod, providing a cost-effective option for consistent workloads.
...
Every Pod comes with pre-configured utility services for development, monitoring, and ML workflows. These services run automatically and are accessible via your browser with no additional setup required.
...
To learn more about all available services and how to use them, see [Built-in utility services](/pods/utility-services).
...
- **Web proxy**: HTTP access to [exposed web services](/pods/configuration/expose-ports) via URLs in the format `https://[pod-id]-[port].brightnode.cloud`.
- **API**: Programmatic access and control through the [Brightnode API](/api-reference/pods/POST/pods).
- **Built-in services**: Pre-configured [utility services](/pods/utility-services) like JupyterLab, VS Code, and monitoring tools.
...
You can transfer data from your Pod to [most major cloud providers](/pods/storage/cloud-sync), and to your local machine using the [Brightnode CLI](/brightnodectl/overview).
...
Brightnode offers two types of Pod:
...
2. Navigate to the [Pod creation page](https://console.brightnode.cloud/pod/create).
...
5. [Manage your Pod](/pods/manage-pods) through the Brightnode console.

## Pricing

Pods are billed by the minute with no fees for ingress/egress. Brightnode also offers long-term [savings plans](/pods/pricing#savings-plans) for extended usage patterns. See [Pod pricing](/pods/pricing) for details.

## Limitations

**Docker Compose is not supported:** Brightnode runs Docker for you, so you cannot spin up your own Docker instance or use Docker Compose on Pods. If your workflow requires Docker Compose, create a custom template with a pre-built Docker image that contains all necessary components.
