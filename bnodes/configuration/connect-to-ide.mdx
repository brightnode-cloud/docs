---
title: "Connect to a Bnode with VSCode or Cursor"
sidebarTitle: "Connect with VSCode/Cursor"
description: "Set up remote development on your Bnode using VSCode or Cursor."
---
...
<Warning>
To enable SSH access, your public key must be present in the `~/.ssh/authorized_keys` file on your Bnode. If you upload your public key to the settings page before your Bnode starts, the system will automatically inject it into that file at startup.

If your Bnode is already running when you upload the key, the system will not perform this injection. To enable SSH access, you'll need to either terminate/redeploy the Bnode, or open a [web terminal](/bnodes/connect-to-a-bnode#web-terminal-connection) on the running Bnode and run the following commands:
...
Next, deploy the Bnode you want to connect to. For detailed deployment instructions, see [Manage Bnodes -> Create a Bnode](/bnodes/manage-bnodes#create-a-bnode).
...
<Tabs>
  ...
  <Tab title="VSCode">
   1. From the [Bnodes](https://www.brightnode.cloud/console/bnodes) page, select the Bnode you deployed.
   ...
   3. Copy the second command, under **SSH over exposed TCP**. It will look similar to this:
      ...
      <Warning>
      If you only see one command under SSH, then SSH over exposed TCP is not supported by your selected Bnode template. This means you won't be able to connect to your Bnode directly through VSCode/Cursor, but you can still connect using [basic SSH](/bnodes/connect-to-a-bnode#basic-ssh-connection) via the terminal.
      </Warning>
...
3. Under **Direct TCP Ports**, look for a line similar to:
   ...
   <Warning>
   If you don't see a **Direct TCP Ports** section, then SSH over exposed TCP is not supported by your selected Bnode template. This means you won't be able to connect to your Bnode directly through VSCode/Cursor, but you can still connect using [basic SSH](/bnodes/configuration/use-ssh#basic-ssh-connection) via the terminal.
   </Warning>
...
3. Choose your Bnode from the list (either by IP or custom name if you configured one).
4. VSCode/Cursor will open a new window and connect to your Bnode.
...
- `/workspace`: Default [persistent storage](/bnodes/storage/types) directory.
...
1. Verify your Bnode is running and fully initialized.
2. Check that your SSH key is properly configured in Brightnode settings.
3. Ensure the Bnode has SSH enabled in its template.

If the VSCode/Cursor server fails to install:

1. Check that your Bnode has sufficient disk space.
2. Ensure your Bnode has internet connectivity.
