---
title: "Sync Bnode data with cloud storage providers"
sidebarTitle: "Sync data with cloud storage"
description: "Learn how to sync your Bnode data with popular cloud storage providers."
---

Brightnode's Cloud Sync feature makes it easy to upload your Bnode data to external cloud storage providers, or download data from cloud storage providers to your Bnode. This guide walks you through setting up and using Cloud Sync with supported providers.
...
Amazon S3 provides scalable object storage that integrates seamlessly with Brightnode through Cloud Sync.
...
<Steps>
  ...
  <Step title="Generate access credentials">
   ...
   <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Bnodes page](https://brightnode.cloud/console/bnodes) and select the Bnode containing your data. Click **Cloud Sync**, then select **AWS S3** from the available providers.
    ...
    Click **Copy to/from AWS S3** to initiate the transfer. The transfer progress will be displayed in the Brightnode interface. Large datasets may take time depending on your Bnode's network connection.
  </Step>
</Steps>
...
<Note>
Cloud Sync is compatible with Google Cloud Storage, but **not Google Drive**. However, you can transfer files between your Bnodes and Drive [using the Brightnode CLI](/bnodes/storage/transfer-files#transfer-files-between-google-drive-and-Brightnode).
</Note>
...
<Steps>
  ...
  <Step title="Set up a service account">
   Create a service account specifically for Brightnode access. This provides better security than using your primary account credentials.
  ...
  <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Bnodes page](https://brightnode.cloud/console/bnodes) and select the Bnode containing your data. Click **Cloud Sync**, then select **Google Cloud Storage** from the available providers.

    Paste the entire contents of your Service Account JSON key into the provided field. Specify the source/destination path within your bucket and select which folders from your Bnode to transfer.

    Click **Copy to/from Google Cloud Storage** to initiate the transfer. The transfer progress will be displayed in the Brightnode interface. Large datasets may take time depending on your Bnode's network connection.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Create a Blob Container">
    Create a Blob Container by going to **Storage Browser → Blob Containers** and clicking **Add Container**. Consider creating folders within the container for better organization if you plan to sync data to/from multiple Bnodes.
  </Step>

  <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Bnodes page](https://brightnode.cloud/console/bnodes) and select the Bnode containing your data. Click **Cloud Sync**, then select **Azure Blob Storage** from the available providers.
    ...
    Click **Copy to/from Azure Blob Storage** to initiate the transfer. The transfer progress will be displayed in the Brightnode interface. Large datasets may take time depending on your Bnode's network connection.
  </Step>
</Steps>
...
<Steps>
  <Step title="Create a B2 bucket">
   ...
   Set the bucket visibility to **Public** to allow Brightnode access. You can restrict access later using application keys if needed.
  </Step>
  ...
  <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Bnodes page](https://brightnode.cloud/console/bnodes) and select the Bnode containing your data. Click **Cloud Sync**, then select **Backblaze B2** from the available providers.

    Enter your **Backblaze B2 Account ID**, **Application Key**, and **bucket path** as shown in the Backblaze interface.

    Click **Copy to/from Backblaze B2** to initiate the transfer. The transfer progress will be displayed in the Brightnode interface.
  </Step>
</Steps>

## Dropbox

Dropbox integration allows you to sync your Bnode data with your Dropbox account using OAuth authentication.
...
<Steps>
  ...
  <Step title="Generate access token">
    ...
    Save this token immediately—it won't be shown again after you leave the page. This token authenticates Brightnode's access to your Dropbox.
  </Step>

  <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Bnodes page](https://brightnode.cloud/console/bnodes) and select the Bnode containing your data. Click **Cloud Sync**, then select **Dropbox** from the available providers.
    ...
    Click **Copy to/from Dropbox** to initiate the transfer. The transfer progress will be displayed in the Brightnode interface.
  </Step>
</Steps>

## Alternative transfer methods

While Cloud Sync provides the easiest way to sync data with cloud providers, you can also transfer files between your Bnode and other destinations using:

- **brightnodectl**: A built-in CLI tool for peer-to-peer transfers using one-time codes.
- **SSH-based tools**: Use SCP or rsync for direct transfers to your local machine.
- **Network volumes**: For persistent storage across multiple Bnodes.

For detailed instructions on these methods, see our [file transfer guide](/bnodes/storage/transfer-files).
...
- **Connection timeouts**: Check that your Bnode has stable network connectivity. You may need to retry the transfer.

For additional support, consult your cloud provider's documentation or contact Brightnode support.
