---
title: "Sync Node data with cloud storage providers"
sidebarTitle: "Sync data with cloud storage"
description: "Learn how to sync your Node data with popular cloud storage providers."
---

Brightnode's Cloud Sync feature makes it easy to upload your Node data to external cloud storage providers, or download data from cloud storage providers to your Node. This guide walks you through setting up and using Cloud Sync with supported providers.
...
<Steps>
  ...
  <Step title="Generate access credentials">
   ...
   <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Nodes page](https://brightnode.cloud/console/nodes) and select the Node containing your data. Click **Cloud Sync**, then select **AWS S3** from the available providers.
    ...
    Click **Copy to/from AWS S3** to initiate the transfer. The transfer progress will be displayed in the Brightnode interface. Large datasets may take time depending on your Node's network connection.
  </Step>
</Steps>
...
<Note>
Cloud Sync is compatible with Google Cloud Storage, but **not Google Drive**. However, you can transfer files between your Nodes and Drive [using the Brightnode CLI](/nodes/storage/transfer-files#transfer-files-between-google-drive-and-Brightnode).
</Note>
...
<Steps>
  ...
  <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Nodes page](https://brightnode.cloud/console/nodes) and select the Node containing your data. Click **Cloud Sync**, then select **Google Cloud Storage** from the available providers.

    Paste the entire contents of your Service Account JSON key into the provided field. Specify the source/destination path within your bucket and select which folders from your Node to transfer.

    Click **Copy to/from Google Cloud Storage** to initiate the transfer. The transfer progress will be displayed in the Brightnode interface. Large datasets may take time depending on your Node's network connection.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Create a Blob Container">
    Create a Blob Container by going to **Storage Browser → Blob Containers** and clicking **Add Container**. Consider creating folders within the container for better organization if you plan to sync data to/from multiple Nodes.
  </Step>

  <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Nodes page](https://brightnode.cloud/console/nodes) and select the Node containing your data. Click **Cloud Sync**, then select **Azure Blob Storage** from the available providers.
    ...
    Click **Copy to/from Azure Blob Storage** to initiate the transfer. The transfer progress will be displayed in the Brightnode interface. Large datasets may take time depending on your Node's network connection.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Nodes page](https://brightnode.cloud/console/nodes) and select the Node containing your data. Click **Cloud Sync**, then select **Backblaze B2** from the available providers.
...
Dropbox integration allows you to sync your Node data with your Dropbox account using OAuth authentication.
...
<Steps>
  ...
  <Step title="Sync your data with Brightnode">
    In the Brightnode console, navigate to the [Nodes page](https://brightnode.cloud/console/nodes) and select the Node containing your data. Click **Cloud Sync**, then select **Dropbox** from the available providers.
...
While Cloud Sync provides the easiest way to sync data with cloud providers, you can also transfer files between your Node and other destinations using:
...
- **Network volumes**: For persistent storage across multiple Nodes.

For detailed instructions on these methods, see our [file transfer guide](/nodes/storage/transfer-files).
...
- **Connection timeouts**: Check that your Node has stable network connectivity. You may need to retry the transfer.
