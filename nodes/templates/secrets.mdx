...
This guide shows how to create, view, edit, delete, and use secrets in your [Node templates](/nodes/templates/overview) to protect sensitive data and improve security.

## What are Brightnode secrets

Secrets are encrypted strings that store sensitive information separately from your template configuration, providing a secure way to store and manage sensitive information such as API keys, passwords, and authentication tokens in your Node templates. 
...
- **Reusability**: The same secret can be referenced across multiple templates and Nodes without duplication.
...
<Warning>
Deleting a secret is permanent and cannot be undone. Make sure no active templates or Nodes are using the secret before deletion, as this will cause those deployments to fail.
</Warning>

## Using secrets in Node templates

Once you've created secrets, you can reference them in your Node templates to provide secure access to sensitive data.

### Direct reference method

Reference your secrets directly in the [environment variables section](/nodes/templates/environment-variables) of your Node template using the `BRIGHTNODE_SECRET_` prefix followed by your secret name:
...
This syntax tells Brightnode to substitute the encrypted secret value when the Node starts, making it available as an environment variable inside your container.

### Web interface selection

When creating or editing a Node template through the web interface, you can also:
