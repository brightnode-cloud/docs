...
Brightnode Public Endpoints provide instant access to state-of-the-art AI models through simple API calls, with an API playground available through the [Brightnode Hub](/hub/overview).
...
1. Navigate to the [Brightnode Hub](https://www.brightnode.cloud/console/hub) in the console.
...
1. Select a model from the [Brightnode Hub](https://www.console.brightnode.cloud/hub).
...
All requests require authentication using your Brightnode API key, passed in the `Authorization` header. You can find and create [API keys](/get-started/api-keys) in the [Brightnode console](https://www.brightnode.cloud/console/user/settings) under **Settings > API Keys**.
...
```bash curl
curl -X POST "https://api.brightnode.ai/v2/black-forest-labs-flux-1-dev/runsync" \
...
```bash curl
curl -X POST "https://api.brightnode.ai/v2/black-forest-labs-flux-1-dev/run" \
  -H "Authorization: Bearer BRIGHTNODE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "input": {
...
```bash curl
curl -X GET "https://api.brightnode.ai/v2/black-forest-labs-flux-1-dev/status/{job-id}" \
  -H "Authorization: Bearer BRIGHTNODE_API_KEY"
```
...
```json
{
  ...
  "output": {
    "cost": 0.02097152,
    "image_url": "https://image.brightnode.ai/6/6/mCwUZlep6S/453ad7b7-67c6-43a1-8348-3ad3428ef97a.png"
  },
  "status": "COMPLETED",
...
headers = {"Content-Type": "application/json", "Authorization": "Bearer BRIGHTNODE_API_KEY"}
...
response = requests.post(
    "https://api.brightnode.ai/v2/black-forest-labs-flux-1-dev/run",
    headers=headers,
    json=data,
)
```
...
For JavaScript and TypeScript projects, you can use the `@Brightnode/ai-sdk-provider` package to integrate Brightnode's Public Endpoints with the [Vercel AI SDK](https://ai-sdk.dev/docs/introduction).

Run this command to install the package:

```bash
npm install @Brightnode/ai-sdk-provider ai
```

To call a Public Endpoint for text generation:

```typescript
import { Brightnode } from '@Brightnode/ai-sdk-provider';
import { generateText } from 'ai';

const { text } = await generateText({
  model: Brightnode('qwen3-32b-awq'),
...
const { image } = await generateImage({
  model: brightnode.imageModel('flux/flux-dev'),
  prompt: 'A serene mountain landscape at sunset',
  aspectRatio: '4:3',
});
```

For comprehensive documentation and examples, see the [Bnode package documentation](https://www.npmjs.com/package/@Brightnode/ai-sdk-provider).
...
<Note>
Brightnode's billing system rounds up after the first 10 decimal places.
</Note>
