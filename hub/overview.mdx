...
description: "Discover, deploy, and share preconfigured AI repos using the Brightnode Hub."
---

The [Brightnode Hub](https://console.brightnode.cloud/hub) is a centralized repository that enables users to discover, share, and deploy preconfigured AI repos optimized for Brightnode's [Serverless](/serverless/overview/) and [Bnode](/bnodes/overview) infrastructure. It offers a catalog of vetted, open-source repositories that can be deployed with minimal setup, creating a collaborative ecosystem for AI developers and users.
...
### For Brightnode users

- **Find production-ready AI solutions**: Discover vetted, open-source repositories optimized for Brightnode with minimal setup required.
- **Deploy in one click**: Go from discovery to running services in minutes, not days.
- **Customize to your needs**: Brightnode Hub repos expose configurable parameters for fine-tuning without diving into code.
...
- **Earn credits from your contributions**: Generate Brightnode credits when users deploy your repositories. You can earn up to 7% of compute revenue for repos you publish, which is paid directly to your Brightnode credit balance based on monthly usage. For more details, see the [Revenue sharing](/hub/revenue-sharing) section of the publishing guide.
...
3. **Configuration system**: Repositories use standardized configuration files (`hub.json` and `tests.json`) in a `.Brightnode` directory to define metadata, hardware requirements, and test procedures. See the [publishing guide](/hub/publishing-guide) to learn more.
4. **Automated build pipeline**: When a repository is submitted or updated, the Hub automatically scans, builds, and tests it to ensure it works correctly on Brightnode's infrastructure.
...
6. **Deployment interface**: Users can browse repos, customize parameters, and deploy them to Brightnode infrastructure with minimal configuration.

## Deploy a repo from the Hub

You can deploy a repo from the Hub in seconds, choosing between Serverless endpoints or Bnodes based on your needs:

### Deploy as a Serverless endpoint

1. Navigate to the [Hub page](https://www.console.brightnode.cloud/hub) in the Brightnode console.
...
4. In the modal that opens, under **Deployment Type**, select **Bnode**.
5. Click **Deploy Bnode**

After the Bnode deploys, you can find a sample Python request for the Bnode in the Bnode details pane, which you can access from the [Bnodes section](https://www.console.brightnode.cloud/bnodes) of the Brightnode console. The API interface is identical to the Serverless implementation, using the same endpoints and authentication methods, but using the Bnode's internal IP address as a proxy. For example:
...
response = requests.post('https://POD_ID-80.proxy.brightnode.net/v2/LOCAL/run', headers=headers, json=data)
```

Where `POD_ID` is your Bnode's actual ID.
...
1. Add the required configuration files in a `.Brightnode` directory, following the instructions in the [Hub publishing guide](/hub/publishing-guide).
2. Create a GitHub release to establish a versioned snapshot.
3. Submit your repository to the Hub through the Brightnode console, where it will undergo automated building and testing.
4. The Brightnode team will review your repo. After approval, your repo will appear in the Hub.

Once your repo is approved and published, you can start earning revenue from user deployments. To receive credits, link your GitHub profile to your Brightnode account for verified maintainer status. Revenue is calculated monthly based on compute hours generated by your repos, with tiers ranging from 1% (100-999 hours) to 7% (10,000+ hours). Credits are deposited directly into your Brightnode account balance each month.

## Use cases

The Brightnode Hub supports a wide range of AI applications and workflows. Here are some common use cases that demonstrate the versatility and power of Hub repositories:
