...
The [Brightnode Hub](https://console.brightnode.cloud/hub) is a centralized repository that enables users to discover, share, and deploy preconfigured AI repos optimized for Brightnode's [Serverless](/serverless/overview/) and [Node](/nodes/overview) infrastructure. It offers a catalog of vetted, open-source repositories that can be deployed with minimal setup, creating a collaborative ecosystem for AI developers and users.
...
You can deploy a repo from the Hub in seconds, choosing between Serverless endpoints or Nodes based on your needs:
...
4. In the modal that opens, under **Deployment Type**, select **Node**.
5. Click **Deploy Node**

After the Node deploys, you can find a sample Python request for the Node in the Node details pane, which you can access from the [Nodes section](https://www.console.brightnode.cloud/nodes) of the Brightnode console. The API interface is identical to the Serverless implementation, using the same endpoints and authentication methods, but using the Node's internal IP address as a proxy. For example:
...
Where `POD_ID` is your Node's actual ID.
