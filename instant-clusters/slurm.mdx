...
This tutorial demonstrates how to configure Brightnode Instant Clusters with [Slurm](https://slurm.schedmd.com/) to manage and schedule distributed workloads across multiple nodes. Slurm is a popular open-source job scheduler that provides a framework for job management, scheduling, and resource allocation in high-performance computing environments. By leveraging Slurm on Brightnode's high-speed networking infrastructure, you can efficiently manage complex workloads across multiple GPUs.

Follow the steps below to deploy a cluster and start running distributed Slurm workloads efficiently.

## Requirements

* You've created a [Brightnode account](https://www.console.brightnode.cloud/home) and funded it with sufficient credits.
* You have basic familiarity with Linux command line.
* You're comfortable working with [Nodes](/nodes/overview) and understand the basics of [Slurm](https://slurm.schedmd.com/).
...
## Step 2: Clone demo and install Slurm on each Node

To connect to a Node:

1. On the Instant Clusters page, click on the cluster you created to expand the list of Nodes.
...
2. In the terminal that opens, run this command to clone the Slurm demo files into the Node's main directory:
...
## Step 4: Install Slurm on each Node

Now run the installation script **on each Node**, replacing `[MUNGE_SECRET_KEY]` with any secure random string (like a password). The secret key is used for authentication between nodes, and must be identical across all Nodes in your cluster.
...
If you're not sure which Node is the primary node, run the command `echo $HOSTNAME` on the web terminal of each Node and look for `node-0`.
...
* **Scale your cluster** by adjusting the number of Nodes to handle larger models or datasets.
