...
- **Automatic role assignment:** Brightnode automatically designates controller and agent nodes.
...
4. Configure your cluster specifications:
   ...
   - **Node count**: Choose the number of Nodes in your cluster.
   ...
   - **Node template**: Select a [Node template](/nodes/templates/overview) or click **Edit Template** to customize start commands, environment variables, ports, or [container/volume disk](/nodes/storage/types) capacity. 
      <Warning>
...
Connect to a node using the **Connect** button, or using any of the [connection methods supported by Nodes](/nodes/connect-to-a-node).
...
While Brightnode's Slurm Clusters work out-of-the-box, you can customize your configuration by connecting to the Slurm controller node using the [web terminal or SSH](/nodes/connect-to-a-node).
