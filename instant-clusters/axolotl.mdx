...
This tutorial demonstrates how to use Instant Clusters with [Axolotl](https://axolotl.ai/) to fine-tune large language models (LLMs) across multiple GPUs. By leveraging PyTorch's distributed training capabilities and Brightnode's high-speed networking infrastructure, you can significantly accelerate your training process compared to single-GPU setups.
...
## Step 2: Set up Axolotl on each Node
...
4. In the terminal that opens, run this command to clone the Axolotl repository into the Node's main directory:
...
## Step 3: Start the training process on each Node

Run this command in the web terminal of **each Node**:
...
After running the command on the last Node, you should see output similar to this after the training process is complete:
...
* **Scale your training** by adjusting the number of Nodes in your cluster (and the size of their containers and volumes) to handle larger models or datasets.
