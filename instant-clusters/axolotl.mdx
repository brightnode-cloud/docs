...
## Step 2: Set up Axolotl on each Bnode
...
4. In the terminal that opens, run this command to clone the Axolotl repository into the Bnode's main directory:
...
## Step 3: Start the training process on each Bnode

Run this command in the web terminal of **each Bnode**:
...
After running the command on the last Bnode, you should see output similar to this after the training process is complete:
...
* **Scale your training** by adjusting the number of Bnodes in your cluster (and the size of their containers and volumes) to handle larger models or datasets.
