...
This tutorial demonstrates how to use Instant Clusters with [PyTorch](http://pytorch.org) to run distributed workloads across multiple GPUs. By leveraging PyTorch's distributed processing capabilities and Brightnode's high-speed networking infrastructure, you can significantly accelerate your training process compared to single-GPU setups.

Follow the steps below to deploy a cluster and start running distributed PyTorch workloads efficiently.

## Step 1: Deploy an Instant Cluster

1. Open the [Instant Clusters page](https://www.console.brightnode.cloud/cluster) on the Brightnode web interface.
2. Click **Create Cluster**.
3. Use the UI to name and configure your Cluster. For this walkthrough, keep **Node Count** at **2** and select the option for **16x H100 SXM** GPUs. Keep the **Node Template** at its default setting (Brightnode PyTorch).
4. Click **Deploy Cluster**. You should be redirected to the Instant Clusters page after a few seconds.

## Step 2: Clone the PyTorch demo into each Node

1. Click your cluster to expand the list of Nodes.

2. Click on a Node, for example `CLUSTERNAME-node-0`, to expand the Node.

3. Click **Connect**, then click **Web Terminal**.

4. In the terminal that opens, run this command to clone a basic `main.py` file into the Node's main directory:

   ```sh
   git clone https://github.com/murat-Brightnode/torch-demo.git
   ```
...
This command launches eight `main.py` processes per node (one per GPU in the Node).
...
After running the command on the last Node, you should see output similar to this:
...
The first number refers to the global rank of the thread, spanning from `0` to `WORLD_SIZE-1` (`WORLD_SIZE` = the total number of GPUs in the cluster). In our example there are two Nodes of eight GPUs, so the global rank spans from 0-15. The second number is the local rank, which defines the order of GPUs within a single Node (0-7 for this example).

The specific number and order of ranks may be different in your terminal, and the global ranks listed will be different for each Node.

This diagram illustrates how local and global ranks are distributed across multiple Nodes:
...
If you no longer need your cluster, make sure you return to the [Instant Clusters page](https://www.console.brightnode.cloud/cluster) and delete your cluster to avoid incurring extra charges.

<Info>

You can monitor your cluster usage and spending using the **Billing Explorer** at the bottom of the [Billing page](https://www.console.brightnode.cloud/user/billing) section under the **Cluster** tab.
...
* **Scale your training** by adjusting the number of Nodes in your cluster to handle larger models or datasets.
