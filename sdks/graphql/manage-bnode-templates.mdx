...
If your container image is private, you can also specify Docker login credentials with a `containerRegistryAuthId` argument, which takes the ID (*not* the name) of the container registry credentials you saved in your Brightnode user settings as a string.
...
curl --request POST \
  --header 'content-type: application/json' \
  --url 'https://api.brightnode.cloud/graphql?api_key=${YOUR_API_KEY}' \
  --data '{"query": "mutation { saveTemplate(input: { containerDiskInGb: 5, dockerArgs: \"python handler.py\", env: [ { key: \"key1\", value: \"value1\" }, { key: \"key2\", value: \"value2\" } ], imageName: \"Brightnode/serverless-hello-world:latest\", isServerless: true, name: \"Generated Serverless Template\", readme: \"## Hello, World!\", volumeInGb: 0 }) { containerDiskInGb dockerArgs env { key value } id imageName isServerless name readme } }"}'
```
...
mutation {
  saveTemplate(input: {
    ...
    imageName: "Brightnode/serverless-hello-world:latest",
    isServerless: true,
...
InGb: 5, dockerArgs: \"python handler.py\", env: [ { key: \"key1\", value: \"value1\" }, { key: \"key2\", value: \"value2\" } ], imageName: \"Brightnode/serverless-hello-world:latest\", name: \"Generated Serverless Template\", volumeInGb: 0, readme: \"## Goodbye, World!\" }) { id containerDiskInGb dockerArgs env { key value } imageName name readme } }"}'
```
...
Note that the template you'd like to delete must not be in use by any Bnodes or assigned to any Serverless endpoints. It can take up to 2 minutes to be able to delete a template after its most recent use by a Bnode or Serverless endpoint, too.

The same mutation is used for deleting both Bnode and Serverless templates.
