...
Note that the template you'd like to delete must not be in use by any Nodes or assigned to any Serverless endpoints. It can take up to 2 minutes to be able to delete a template after its most recent use by a Node or Serverless endpoint, too.

The same mutation is used for deleting both Node and Serverless templates.
