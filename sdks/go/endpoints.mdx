...
Interacting with Brightnode's Endpoints is a core feature of the SDK, enabling the execution of tasks and the retrieval of results. This section covers the synchronous and asynchronous execution methods, along with checking the status of operations.

## Prerequisites

Before using the Brightnode Go SDK, ensure that you have:

* [Installed the Brightnode Go SDK](/sdks/go/overview#install).
* Configured your API key.

## Set your Endpoint Id

Set your Brightnode API key and your Endpoint Id as environment variables.
...
func main() {
	...
	if apiKey == "" {
		log.Fatalf("Environment variable BRIGHTNODE_API_KEY is not set")
	}
	if baseURL == "" {
		log.Fatalf("Environment variable BRIGHTNODE_BASE_URL is not set")
	}
